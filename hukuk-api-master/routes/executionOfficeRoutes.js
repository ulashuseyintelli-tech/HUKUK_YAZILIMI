const express = require("express"),
  ExecutionOffice = require("../models/ExecutionOffice"),
...
router.get("/", Middlewares.verifyToken, (req, res, next) => {
  ...
  ExecutionOffice.find({ lawOfficeId })
...
router.post("/", Middlewares.verifyToken, (req, res, next) => {
  const { lawOfficeId } = res.locals.decoded;
  ExecutionOffice.create({ lawOfficeId, ...req.body })
...
router.put("/:officeId", Middlewares.verifyToken, (req, res, next) => {
  ...
  ExecutionOffice.findByIdAndUpdate(req.params.officeId, body, { new: true })
    .then((office) => {
      res.send(office);
    })
    .catch((e) => next(serverError(e)));
