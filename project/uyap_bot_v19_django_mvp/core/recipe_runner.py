...
def _get_ui_worker(uimap_pack: Dict[str, Any]) -> UiWorker:
    if os.environ.get("REAL_UI", "0") == "1":
        ...
        w = PlaywrightUiWorker(uimap_pack, cfg)
        w.start()
        return w
...
def run_recipe(job: JobRun, ui_worker: Optional[UiWorker] = None) -> Dict[str, Any]:
    ...
    # If degraded mode: block write-ish actions by policy
    ...
    ui_worker = ui_worker or _get_ui_worker(uimap_pack)
    ...
    try:
        ...
        meta_payload = {"recipe_id": job.recipe_id, "recipe_version": recipe.get("version"), "uyap_nav_path": nav_path, "runner": "v19", "degraded_mode": is_degraded_mode()}
