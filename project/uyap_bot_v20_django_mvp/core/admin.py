...
from .models import Case, Debtor, Asset, Lien, Fact, Snapshot, JobRun, JobStep, Lock, Communication, RecipeBundle, ParamBundle, UiMapBundle, EvidenceExport, SelectorHealthLog
...
@admin.register(EvidenceExport, SelectorHealthLog)
class EvidenceExport, SelectorHealthLogAdmin(admin.ModelAdmin):
    list_display = ("id", "case", "status", "export_hash", "created_at")
    list_filter = ("status",)


@admin.register(SelectorHealthLog)
class SelectorHealthLogAdmin(admin.ModelAdmin):
