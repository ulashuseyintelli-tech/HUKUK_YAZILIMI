...
from .models import Case, Debtor, Asset, Lien, Fact, Snapshot, JobRun, JobStep, Lock, Communication, RecipeBundle, ParamBundle, UiMapBundle, EvidenceExport, SelectorHealthLog, SystemConfig
...
class RecipeBundleAdmin(admin.ModelAdmin):
    list_display = ("id", "name", "version", "status", "content_hash", "updated_at")
    search_fields = ("name",)
    list_filter = ("status",)
...
@admin.register(EvidenceExport, SelectorHealthLog, SystemConfig)
class EvidenceExport, SelectorHealthLog, SystemConfigAdmin(admin.ModelAdmin):
    list_display = ("id", "case", "status", "export_hash", "created_at")
    list_filter = ("status",)


@admin.register(SelectorHealthLog, SystemConfig)
class SelectorHealthLog, SystemConfigAdmin(admin.ModelAdmin):
