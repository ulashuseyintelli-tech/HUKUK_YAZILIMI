...
from core.decision_engine import run_decision_rules
                ...
                if isinstance(extractor_specs, list) and extractor_specs:
                    ...
                    if created > 0:
                        ...
                        for ft in [ft for ft in fact_types if ft]:
                            # enqueue based on actual fact rows created from this snapshot
                            for fact in Fact.objects.filter(case=job.case, snapshot=snap, fact_type=ft).order_by('-created_at')[:50]:
                                run_decision_rules(job.case, job.debtor, fact)
