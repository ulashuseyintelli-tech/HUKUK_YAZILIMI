...
- [x] 1. Alert rule ekle — `ops/prometheus/redrive-alerts.yml`
  - [x] 1.1 `RedriveScrapeDown` alert rule'unu mevcut `redrive_alerts` group'una ekle
...
- [x] 2. Ops doc güncelle — `docs/redrive-ops-runbook.md`
  - [x] 2.1 §5 Scrape Health / RedriveScrapeDown bölümünü ekle
...
- [x] 3. Testleri güncelle — `redrive-ops-artifacts.spec.ts`
  - [x] 3.1 `EXPECTED_ALERT_NAMES` listesine `'RedriveScrapeDown'` ekle
  - [x] 3.2 Per-alert severity spot check: `RedriveScrapeDown` → `severity: critical` testi ekle
  - [x] 3.3 Property 3 design matrix: `{ alert: 'RedriveScrapeDown', sectionFragment: 'scrape-health' }` ekle
  - [x] 3.4 Property 3 reverse mapping: §5 bölümünü orphan kontrolüne dahil et (yeni alert §5'e link veriyor — orphan olmamalı)
  - [x] 3.5 Task 4.4 ops doc içerik doğrulama: `PLAYBOOK_SECTIONS` listesine §5 ekle (veya §5'i hariç tut — karar: §5 tam playbook formatında ise dahil et)
  - _Requirements: 4.1, 4.2, 4.3, 4.4_

- [x] 4. Checkpoint — Tutarlılık kontrolü
