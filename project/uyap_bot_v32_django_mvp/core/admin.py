...
from .models import Case, Debtor, Asset, Lien, Fact, Snapshot, JobRun, JobStep, Lock, Communication, RecipeBundle, ParamBundle, UiMapBundle, EvidenceExport, SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause
...
@admin.register(EvidenceExport, SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause)
class EvidenceExport, SelectorHealthLog, SystemConfig, CaseRunLock, RecipePauseAdmin(admin.ModelAdmin):
    list_display = ("id", "case", "status", "export_hash", "created_at")
    list_filter = ("status",)


@admin.register(SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause)
class SelectorHealthLog, SystemConfig, CaseRunLock, RecipePauseAdmin(admin.ModelAdmin):
...
@admin.register(SystemConfig, CaseRunLock, RecipePause)
class SystemConfig, CaseRunLock, RecipePauseAdmin(admin.ModelAdmin):
    list_display = ("id","key","updated_at")
    search_fields = ("key",)


@admin.register(CaseRunLock, RecipePause)
class CaseRunLock, RecipePauseAdmin(admin.ModelAdmin):
    list_display = ("id","case","is_locked","locked_by_job_id","lock_reason","updated_at")
    list_filter = ("is_locked",)


@admin.register(RecipePause)
class RecipePauseAdmin(admin.ModelAdmin):
