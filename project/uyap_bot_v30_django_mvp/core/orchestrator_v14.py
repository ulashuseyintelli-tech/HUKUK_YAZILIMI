        ...
        if scope == "debtor":
            for d in case.debtors.all():
                if _recent_exists(case, d, rid, interval):
                    continue
                jobs.append(JobRun.objects.create(case=case, debtor=d, recipe_id=rid, recipe_version=1, status=JobStatus.QUEUED, risk_level=rl))
        else:
            if _recent_exists(case, None, rid, interval):
                continue
            jobs.append(JobRun.objects.create(case=case, debtor=None, recipe_id=rid, recipe_version=1, status=JobStatus.QUEUED, risk_level=rl))
