...
from .models import Case, Debtor, Asset, Lien, Fact, Snapshot, JobRun, JobStep, Lock, Communication, RecipeBundle, ParamBundle, UiMapBundle, EvidenceExport, SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause, UiMapRecording
...
@admin.register(EvidenceExport, SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause, UiMapRecording)
class EvidenceExport, SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause, UiMapRecordingAdmin(admin.ModelAdmin):
    list_display = ("id", "case", "status", "export_hash", "created_at")
    list_filter = ("status",)


@admin.register(SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause, UiMapRecording)
class SelectorHealthLog, SystemConfig, CaseRunLock, RecipePause, UiMapRecordingAdmin(admin.ModelAdmin):
...
@admin.register(SystemConfig, CaseRunLock, RecipePause, UiMapRecording)
class SystemConfig, CaseRunLock, RecipePause, UiMapRecordingAdmin(admin.ModelAdmin):
    list_display = ("id","key","updated_at")
    search_fields = ("key",)


@admin.register(CaseRunLock, RecipePause, UiMapRecording)
class CaseRunLock, RecipePause, UiMapRecordingAdmin(admin.ModelAdmin):
    list_display = ("id","case","is_locked","locked_by_job_id","lock_reason","updated_at")
    list_filter = ("is_locked",)


@admin.register(RecipePause, UiMapRecording)
class RecipePause, UiMapRecordingAdmin(admin.ModelAdmin):
    list_display = ("id","recipe_id","is_paused","reason","updated_at")
    list_filter = ("is_paused",)
    search_fields = ("recipe_id","reason")


@admin.register(UiMapRecording)
class UiMapRecordingAdmin(admin.ModelAdmin):
    list_display = ("id","label","selector","approved","updated_at")
    list_filter = ("approved",)
    search_fields = ("label","selector")
