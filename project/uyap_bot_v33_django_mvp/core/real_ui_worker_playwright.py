...
from core.table_parser import read_table_by_columns
...
class PlaywrightUiWorker(UiWorker):
    ...
    def _resolve(self, key: str) -> str:
        return get_selector(self.uimap, key)
    ...
    def read_table(self, table_key: str, filters: Optional[Dict[str, Any]] = None) -> UiResult:
        try:
            # if uimap includes a structured columns_keys for this table, use it
            columns_keys = None
            screens = (self.uimap.get("ui_map") or {}).get("screens") or {}
            for _, spec in screens.items():
                table = spec.get("table") or {}
                if table.get("rows") == table_key and isinstance(table.get("columns_keys"), dict):
                    columns_keys = table["columns_keys"]
                    break

            if columns_keys:
                rows = read_table_by_columns(self.page, self.uimap, table_key, columns_keys)
                if filters:
                    def ok(r):
                        return all(str(v) in str(r.get(k, "")) for k, v in filters.items())
                    rows = [r for r in rows if ok(r)]
                sp = self._screenshot("table_cols")
                self._log_health(table_key, True, None, sp)
                return UiResult(ok=True, data={"table": table_key, "rows": rows, "row_count": len(rows)}, evidence={"screenshot_path": sp})
            else:
                row_sel = self._resolve(table_key)
                rows_loc = self.page.locator(row_sel)
                n = rows_loc.count()
                out = []
                for i in range(n):
                    txt = rows_loc.nth(i).inner_text()
    ...
    def wait_for(self, selector_key: str, timeout_ms: int = 10000) -> UiResult:
        try:
            ...
            return UiResult(ok=True, data={"waited_for": selector_key, "timeout_ms": timeout_ms}, evidence={"screenshot_path": sp})
        except Exception as e:
