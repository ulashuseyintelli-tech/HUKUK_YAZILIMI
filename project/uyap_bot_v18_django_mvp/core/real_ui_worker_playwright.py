...
class PlaywrightConfig:
    ...
    slow_mo_ms: int = 0
    base_url: Optional[str] = None  # if you have a landing page
    screenshot_dir: str = "exports/evidence"

class PlaywrightUiWorker(UiWorker):
    """Generic UI worker using Playwright.

    IMPORTANT:
    - This is a framework adapter. You must provide real selectors in UiMapBundle.content
      under `locator_bindings` (see locator_resolver.py).
    - No UYAP-specific selectors are embedded here.
    - Recommended: persistent context with a user profile that already has required auth.
    """
    ...
    def navigate(self, nav_path: List[str]) -> UiResult:
        # Generic: try to click through menu items defined by screen nav_path keys, else no-op.
        try:
            # If uimap has a screen matching nav_path, use its menu selectors (optional).
            ...
            # optional: menu_clicks: ["BTN_MENU_TEBLIGAT", "BTN_SUB_HAZIRLANMIS"]
            ...
            return UiResult(ok=True, data={"nav_path": nav_path, "screen": " > ".join(nav_path)}, evidence={"screenshot_path": self._screenshot("navigate")})
    ...
    def fill_form(self, fields: Dict[str, Any]) -> UiResult:
        try:
            for key, value in fields.items():
                sel = get_selector(self.uimap, key)  # field key
    ...
    def read_table(self, table_key: str, filters: Optional[Dict[str, Any]] = None) -> UiResult:
        try:
            ...
            n = rows.count()
            # Minimal: return raw text rows; parsing can be expanded per table spec.
            out = []
            for i in range(n):
                txt = rows.nth(i).inner_text()
                out.append({"row_index": i, "text": txt})
            # Apply simple filter: keep rows containing all filter values as substrings
            if filters:
                def ok(r):
    ...
    def select_row(self, table_key: str, where: Dict[str, Any]) -> UiResult:
        # naive: pick first row whose text contains all where values, then click it.
        try:
            row_sel = get_selector(self.uimap, table_key)
            rows = self.page.locator(row_sel)
            n = rows.count()
