...
def risk_scoring(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    w = {"rank":0.35, "prior_claims":0.20, "uncertainty":0.20, "value_confidence":0.15, "lien_activity":0.10}
    score = (
        w["rank"]*rank_risk +
    ...
    score = round(score, 1)
    return {"score": score, "components": {"rank": rank_risk, "prior_claims": prior_claims_risk, "uncertainty": uncertainty_risk, "value_confidence": value_confidence_risk, "lien_activity": lien_activity_risk}}

def recovery_simulator(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    min_net_for_cost_actions = float(context.get("min_net_for_cost_actions") or 25000)
    ...
    out["flags"] = {"ok_for_cost_actions": out["expected_net_mid"] >= min_net_for_cost_actions}
    return out
