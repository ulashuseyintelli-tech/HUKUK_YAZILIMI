...
export function readClientWorkspaceSmokeConfig(env: NodeJS.ProcessEnv = process.env): ClientWorkspaceSmokeConfig {
  return {
    ...
    webBaseUrl: trimTrailingSlash(env.CLIENT_WORKSPACE_WEB_BASE_URL ?? 'http://127.0.0.1:3002'),
    apiBaseUrl: trimTrailingSlash(env.CLIENT_WORKSPACE_API_BASE_URL ?? env.NEXT_PUBLIC_API_URL ?? 'http://127.0.0.1:8080'),
