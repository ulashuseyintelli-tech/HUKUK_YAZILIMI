...
describe("AuthService — K1-7 passwordHash nullable guard", () => {
  it("[1] mevcut (passwordHash dolu + aktif) kullanıcı login olur", async () => {
    const { svc } = make({ id: "u1", tenantId: "t1", email: "a@x.com", passwordHash: HASH, role: "ADMIN", isActive: true, tenant: { lifecycle: "ACTIVE" } });
  ...
  it("[2] passwordHash=null kullanıcı login OLAMAZ (bcrypt crash YOK → Unauthorized)", async () => {
    ...
    const { svc } = make({ id: "u1", tenantId: "t1", email: "a@x.com", passwordHash: null, role: "USER", isActive: true, tenant: { lifecycle: "ACTIVE" } });
  ...
  it("[3] isActive=false kullanıcı login OLAMAZ", async () => {
    const { svc } = make({ id: "u1", tenantId: "t1", email: "a@x.com", passwordHash: HASH, role: "USER", isActive: false, tenant: { lifecycle: "ACTIVE" } });
  ...
  it("[24] yanlış parola yine reddedilir (mevcut davranış korunur)", async () => {
    const { svc } = make({ id: "u1", tenantId: "t1", email: "a@x.com", passwordHash: HASH, role: "USER", isActive: true, tenant: { lifecycle: "ACTIVE" } });
