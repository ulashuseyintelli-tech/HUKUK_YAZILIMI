...
export class AddressTaskSchedulerService {
   ...
   */
  @Cron(CronExpression.EVERY_HOUR, { name: 'AddressTaskSchedulerService.checkOverdueTasks', timeZone: SCHEDULER_TIMEZONE })
  async checkOverdueTasks(): Promise<void> {
    const result = await runWithOverlapGuard('AddressTaskSchedulerService.checkOverdueTasks', async () => {
    ...
    if (result === 'SKIPPED_ALREADY_RUNNING') {
      this.logger.warn('[scheduler] AddressTaskSchedulerService.checkOverdueTasks already running, skipping');
    }
  }
   ...
   */
  @Cron(CronExpression.EVERY_DAY_AT_MIDNIGHT, { name: 'AddressTaskSchedulerService.checkAnnualRefreshTasks', timeZone: SCHEDULER_TIMEZONE })
  async checkAnnualRefreshTasks(): Promise<void> {
    const result = await runWithOverlapGuard('AddressTaskSchedulerService.checkAnnualRefreshTasks', async () => {
    ...
    if (result === 'SKIPPED_ALREADY_RUNNING') {
      this.logger.warn('[scheduler] AddressTaskSchedulerService.checkAnnualRefreshTasks already running, skipping');
    }
  }
   ...
   */
  @Cron(CronExpression.EVERY_5_MINUTES, { name: 'AddressTaskSchedulerService.publishOutboxEvents', timeZone: SCHEDULER_TIMEZONE })
  async publishOutboxEvents(): Promise<void> {
    const result = await runWithOverlapGuard('AddressTaskSchedulerService.publishOutboxEvents', async () => {
    ...
    if (result === 'SKIPPED_ALREADY_RUNNING') {
      this.logger.warn('[scheduler] AddressTaskSchedulerService.publishOutboxEvents already running, skipping');
    }
  }
