...
describe("DebtorService.updateQuickNote — Gate-3 passive writer hardening", () => {
  ...
  it("NEGATİF: string-dışı text payload reddedilir ve update çağrılmaz", async () => {
    const prisma = makePrisma();
    prisma.caseDebtor.findFirst.mockResolvedValue({ id: CD });
    const guard = makeGuard(false);
    const svc = makeSvc(prisma, guard);
    ...
    await expect(
      svc.updateQuickNote(TENANT, CASE, CD, "u1", ["not"] as any),
    ).rejects.toThrow("Not metin olmalıdır");
