...
def export_case_audit(case: Case, requested_by: str="system") -> EvidenceExport:
    ...
    package = {
        ...
        "snapshots": [ {"id": s.id, "source": s.source, "uyap_nav_path": s.uyap_nav_path, "snapshot_hash": s.snapshot_hash, "created_at": s.created_at.isoformat(), "screenshot_path": s.screenshot_path} for s in snapshots ],
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        # include screenshots if exist
        for s in snapshots:
            if s.screenshot_path and os.path.exists(s.screenshot_path):
                arc = os.path.join("evidence", os.path.basename(s.screenshot_path))
                z.write(s.screenshot_path, arcname=arc)
