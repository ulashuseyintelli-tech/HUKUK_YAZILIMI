...
function advanceOriginBehindOurBack(originDir) {
  ...
  // --branch main and explicit refspecs throughout: a bare repo's own HEAD
  // symref is fixed at `git init --bare` time from init.defaultBranch (or
  // hard-defaults to "master" when that's unset), independent of which
  // branch actually got pushed into it later. On a runner where that
  // default is "master", a bare `git clone` here would check out an unborn
  // "master" (no commits — "main" exists but isn't HEAD) and every
  // subsequent op would silently operate on the wrong, unrelated history,
  // making the later push fail as "non-fast-forward" for the wrong reason.
  // Confirmed via a real CI failure on ubuntu-latest that this repo's local
  // git config didn't reproduce.
  git(['clone', '--quiet', '--branch', 'main', originDir, cloneDir]);
  ...
  git(['push', '--quiet', 'origin', 'main:refs/heads/main'], cloneDir);
  cleanup(cloneDir);
}
