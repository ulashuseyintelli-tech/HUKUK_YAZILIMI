...
| SB-002 | **IR-0 → PartyMatch** (kimlik çözümleme motoru) | HOLD | Party Faz 5 İÇİNDE inşa edilecek; standalone YAPILMAYACAK (DebtorIdentityCandidate ⊂ PartyMatchCandidate). **Kapsar:** "update'te mevcut'a merge" UX boşluğu (bugün tüm modüllerde update = override-or-cancel; "mevcut kayda birleştir" YOK → PartyMatch merge ile çözülür) | `debtor-identity-resolution-ir0.md` (#140) |
...
| SB-013 | **Web CI'a `next build` gate eklenmesi** | ✅ DONE | **KAPANDI (2026-06-25):** `#491` → `cb2203c` — web-tests job'a `pnpm --filter @hukuk/web build` adımı eklendi (yeni job değil; required check "Web Tests (vitest)" artık vitest + build çalıştırır → frontend boot kırığı main'e giremez). Build step CI'da fiilen koştu+geçti. **Orijinal problem (kayıt):** vitest geçtiği halde Next route/build hatası main'e girdi (#185); `/intake/[token]` vs `(dashboard)/intake/[id]` slug conflict → web BOOT olmadı ama CI yeşildi (postmortem FM: test geçti ≠ uygulama çalışıyor). | ✅ `#491`→cb2203c · postmortem · #185 |
| SB-011 | **Calc / Faiz / TBK100 reliability audit turu** | HOLD | Ayrı uzman alan (para/faiz hesapları); bu audit kapsamı dışındaydı. Reliability Audit deseniyle (ledger + canlı doğrulama) ayrı tur gerekir | reliability-ledger HENÜZ TARANMADI |
| SB-012 | **Soft-delete model TAM sweep** (audit thoroughness) | READY | Audit yüksek-trafikli modelleri taradı (Client/Debtor/Lawyer/Staff/Lookup/Group/Portal); kalan tüm soft-delete eden modeller eksiksiz enumerate edilmedi → düşük öncelik tamamlama | reliability-ledger HENÜZ TARANMADI |
