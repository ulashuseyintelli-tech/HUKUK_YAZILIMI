...
| SB-013 | **Web CI'a `next build` gate eklenmesi** | READY | **Problem:** vitest geçtiği halde Next route/build hatası main'e girdi (#185 kanıt). **Örnek:** `/intake/[token]` ile `(dashboard)/intake/[id]` slug conflict → web BOOT olmadı ama CI yeşildi. **Risk:** frontend boot olmasa bile CI yeşil görünüyor (postmortem FM: test geçti ≠ uygulama çalışıyor). **Öneri:** web CI'a `pnpm --filter @hukuk/web build` (veya eşdeğer `next build`) gate ekle. Party'den bağımsız, küçük workflow değişikliği | postmortem · #185 |
