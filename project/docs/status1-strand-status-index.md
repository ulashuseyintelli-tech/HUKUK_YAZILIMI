...
## 5. Re-sync (2026-06-25 · main `0ab3cac`)

> **Tür:** docs-only status resync. Kod/test/workflow/migration YOK. §1-4'ün üzerine güncel canlı durumu ekler;
> orijinal 2026-06-24 / `1d4dfd2` kaydı tarihsel olarak korunur (otorite = git).

`1d4dfd2` (CI-1 `#462`) → `0ab3cac` arasında kapanan/değişen durumlar:

### 5.1 Yeni KAPALI strand — WP-1d-5 Legal Responsibility write-path
- **Durum: KAPALI** (controlled-minimum checkpoint, kapanış notu `#486`).
- Kanıt zinciri: contract `#470` · matrix `#471` · product/legal `#472` · endpoint audit `#473` ·
  kanonik backend endpoint `#474` · UI `#476`/`#477` · UI-bypass closure `#479` · backend-bypass closure `#480` ·
  lifecycle decision `#482` · lifecycle hardening `#483` · D4 zero-responsible audit `#485` · closure note `#486`.
- Özet: Kanonik write-path (`PATCH /cases/:id/legal-responsible-lawyer` — ADMIN + reason + 409 + changeType audit)
  tek yazı kapısı; generic update + lifecycle silent replacement/removal bypass'ları kapatıldı; D4 dev-veri temiz;
  initial-init + create-dedupe + ownerless = kabul edilmiş istisnalar.
- Dil kuralı: **"tüm legal-responsible değişimleri sonsuza dek yalnız canonical'dan" DEME** (kabul edilmiş istisnalar +
  gerçek-veri owner-run audit caveat'i var). Yeni WP-1d-5 gate'i yalnız somut bug / prod owner-run bulgusu /
  açık ürün-hukuk kararı ile açılır. Full RBAC/permission-store/role-template-UI ihtiyaç-gated [WP-4z].

### 5.2 §4 sıradaki-gate önerilerinin akıbeti
- **(1) Deprecated `addressType`/`isMernis` forensic → ✅ DONE** (zaten ADDR-1; residual=0 teyidi + ADDR-1-FU `#489`).
  DB kolon DROP / DTO-UX migrasyonu açılmadı (owner-run / opsiyonel).
- **(2) CaseDetailTabs cleanup FORENSIC → ✅ DONE** (CDT-1 forensic `#467` + cb2203c re-verify `#495`).
  **Gerçek cleanup hâlâ `REWIRE_REQUIRED_BEFORE_CLEANUP`** (C3 UYAP + orphaned-panel ürün kararı gerekir; bu index açmaz).
- **(3) Scheduler NAFAKA `DueType.PRINCIPAL` fix → Codex domain'e devredildi (SCHED-1).** Bu index'in "sıradaki
  küçük gate"i DEĞİL; NAFAKA/scheduler/DueType Codex alanı. Contract-note merged (`docs: SCHED-1 NAFAKA DueType contract`).

### 5.3 CI/kalite (strand değil, kapanan iş)
- **SB-013** Web CI `next build` gate → ✅ DONE `#491` → `cb2203c` (web-tests artık vitest + build). [strategic-backlog DONE'a çekildi]
- **debtor-wizard-accept flake** → kökten deterministik fix `#494` → `b2724a7` (timeout-bump değil; "Tümünü Ekle"
  öncesi partyRows effect-population yarışı, olay-tabanlı bekleme ile çözüldü).

### 5.4 §3 in-flight tazeleme
- `#463` (cutover-readiness docs) → artık **MERGED** (`§3`'te "open" listelenmişti, bayat). Güncel açık PR'lar:
  `#406` (claim-item-wizard multi-item) · `#407` (BALANCE-DISPLAY PR-1b) · `#408` (A1d-pre G1 OCR) —
  **hepsi başka strand / Codex-balance; bu index karar vermez (yalnız görünürlük).**
- Bayat worktree `casedetailtabs-migration-cleanup-a` → bu oturumda **temizlendi** (içindeki commit'siz erken Cleanup-A
  WIP repo-dışı patch'e yedeklendi + `apply --check` geçti; worktree + local branch silindi; kanonik repo bütün).

**WP-1d-4 / WP-4 / CI-1 KAPALI kalır; WP-4'e dönülmez. Bu re-sync docs-only — yeni gate açmaz.**
