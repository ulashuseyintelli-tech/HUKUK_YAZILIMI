...
**Status:** APPROVED / POST-PR-10 BASELINE EXECUTION PLAN v2.27
**Date:** 2026-07-10
...
> **v2.23 (2026-07-13):** ADR014-PE-06D technical/governance closure reconciliation. Technical
> PR #1203 (head `075a079a2b680abf246bd9b173fc3e77bcb89ddf`; squash
> `2d626b156cab5ed726c2ac9ae551220e8d51392b`) adds a default-disabled, synthetic-only local
> session-orchestrator dry-validation shell. In explicit `TEST_ONLY` mode it first requires the
> existing PE-06A preparation gate, then validates deterministic session/phase ordering, obtains
> terminal phase duration only from an injected monotonic test clock, and reuses PE-06B1 factories
> plus PE-06C1 v2 event/metric mappers. Success, failure, timeout, cancellation, abort, invalid
> transition, missing authorization and disabled no-emission paths are covered. There is no
> production call-site, real/local-production data access, runtime telemetry activation,
> persistence, external egress, evidence execution, financial/readiness/API behavior or authority
> change. Synthetic `DRY_VALIDATED` output is not representative evidence. Representative evidence
> remains absent/blocking; CAN-CUT-02 remains open; PR-11 and runtime cutover remain NOT AUTHORIZED.
...
> **v2.25 (2026-07-13):** ADR014-REP-01B technical/governance closure reconciliation. Technical
> PR #1211 (head `e34faea3a45127edd29d15cb5b7bd7f91448ca53`; squash
> `a4c72414ab3cea26e36cd9b6c42735024c01db7b`) adds the exact default-disabled local read-only
> representative runner boundary. The runner composes PE-06A/PE-06D, requires bound canonical SHA,
> environment, session, manifest, access and execution authorization references, enforces
> `REPEATABLE READ / READ ONLY`, exposes only a guarded query port, has no egress dependency,
> supports deterministic abort and writes a create-once JSON artifact under an owner-controlled
> local root. The artifact remains `CAPTURED_NOT_ACCEPTED / authority=NONE / official=false` and
> does not accept representative evidence or authorize runtime. REP-01A remains `OPEN / BLOCKED`
> until the specific run package closes; representative execution/evidence remain absent,
...
> **v2.27 (2026-07-13):** ADR014-REP-01A-R2 closes the circular eligibility deadlock in the
> preparation-only authorization contract. Technical PR #1219 (head
> `82ccff90f1734a623ef0928dcba6ab40c7ecd1b5`; squash
> `3421f5c46b8b32268731fa6d96c72fd290c416b4`) preserves the v1 one-shot API unchanged and adds
> a v2 `PRE_RUN_AUTHORIZED → RUNTIME_BINDING_REQUIRED → CAPTURE_COMPLETE` sequence. Exact
> environment, operator, reviewer-assignment policy, separate access/execution approvals,
> read-only/no-egress controls, output, owner-controlled indefinite retention, manifest/baseline
> methods and five owner sign-offs are pre-run requirements. Session, approved manifest instance,
> actual reviewer, UTC windows and observed counts are runtime/post-capture facts. Missing pre-run
> decisions block execution; missing runtime facts do not invalidate pre-run authorization but do
> block `CAPTURE_COMPLETE` and evidence acceptance. No data was accessed, no run/package instance
> was created and no REP-02, evidence, PR-11, runtime or cutover authority was granted. The next
> eligible workstream is a separately owner-authorized v2 pre-run package-instance materialization;
> representative evidence remains absent/blocking.

> **v2.28 (2026-07-14):** ADR014-REP-01A-R2-I1 materializes the first repository-backed v2
> pre-run owner-decision instance. Technical PR #1224 (head
> `b4f88941360e7442430d98183f8967a3780f9312`; squash
> `d15f35a327ea3ddda74a15cfb98414568214a32a`) binds the approved local Windows 11/same-host
> PostgreSQL environment, full eligible population/no sampling, owner operator/evidence ownership,
> reviewer-assignment policy, separate read-only access and single-run execution approvals,
> no-write/no-egress controls, create-once local output, indefinite owner-controlled retention,
> five sign-offs and baseline method to deterministic opaque references. The verified canonical
> HEAD is supplied at materialization because a committed file cannot contain its own future merge
> SHA. A valid materialization returns `PRE_RUN_AUTHORIZED / RUNTIME_BINDING_REQUIRED`; exact
> manifest/approval, reviewer actor, session, UTC windows, counts and capture hash remain absent.
> No database access, run, evidence acceptance, REP-02 authorization, PR-11 readiness or cutover
> authority was produced. The next step is an explicit owner decision for REP-02 execution.
...
Run authorization mechanism   REP-01A-R2 CLOSED / CANONICAL / PHASED-CONTRACT-ONLY
Pre-run package instance       PRESENT / PRE_RUN_AUTHORIZED / RUNTIME_BINDING_REQUIRED
Runtime binding                ABSENT / FUTURE AUTHORIZED RUN FACTS REQUIRED
Next eligible step             OWNER DECISION REQUIRED / ADR014-REP-02 EXECUTION
...
| 11 | `UNASSIGNED` cutover authorization | PR-10 technical + governance closure (**SATISFIED**) | Rollback, monitoring, audit, signoff and acceptance policy | Governance decision record → **`docs/design/adr-014-cutover-authorization-policy.md`**; PE-01/PE-01A zero-cent contract/alignment; PE-02 procedure; PE-03 environment contract; PE-04 dataset/manifest contract; PE-05 → **`docs/design/adr-014-metrics-audit-dashboard-alert-operational-contract.md`** | Required | **POLICY + PRE-EVIDENCE CONTRACTS + REP-01A-R2/REP-01B MECHANISMS + R2-I1 PRE-RUN INSTANCE CLOSED / EVIDENCE ENV = LOCAL / STILL OWNER-GATED** — PE-01/01A/02/03/04/05, PE-05A1a–A4, PE-05B and PE-06A–E remain canonical; REP-01B provides only the default-disabled read-only local runner, REP-01A-R2 provides the backward-compatible phased contract, and R2-I1 records the first owner-decision instance. `PRE_RUN_AUTHORIZED`, `RUNTIME_BINDING_REQUIRED`, `CAPTURE_COMPLETE`, `PREPARED`, `DRY_VALIDATED`, `PACKAGE_COMPLETE` and `CAPTURED_NOT_ACCEPTED` are not evidence acceptance, PR-11 readiness or runtime authority. Runtime binding, dataset materialization, measured baseline and representative evidence remain absent; remaining cutover gate = explicit REP-02 owner GO + authorized local run/runtime binding + validated/accepted local representative evidence + explicit owner `APPROVED`; PR-11 remains NOT AUTHORIZED; next eligible action is owner decision for REP-02 only |
