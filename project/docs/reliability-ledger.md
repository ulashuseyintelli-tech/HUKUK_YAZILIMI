...
|----|-------|-------|-------|------|---------------|
| **RFA-018** ✅ **RESOLVED** | **CI frontend/web testlerini HİÇ koşmuyordu** — `apps/web` vitest CI dışındaydı. Sonuç: (1) kırmızı web testleri merge olabilir, (2) component regresyonları görünmez, (3) CI yalnız backend (api) sağlığını doğrular | `.github/workflows/ci.yml`: önceden yalnız api guardrails + `tsc.prod` + jest. **Canlı kanıt:** `cross-check.test.ts` ÖNCEDEN kırmızı ama tüm CI koşuları (PR #169 dahil) yeşil geçti | **A** | **MED→kapandı** | **RESOLVED**: önce cross-check kırmızısı PR #171'de çözüldü (FORM_6 önceliği; web suite 145/145) → SONRA `web-tests` job (vitest, jsdom; DB/prisma gerekmez; `pnpm --filter @hukuk/web test`) ci.yml'e eklendi (bu PR). Sıra korundu (yeşil suite üzerine eklendi → ilk koşuda bloklamadı). Artık kırmızı web testi merge'i bloklar. |
