...
| **D4** | DB-level exactly-one hardening (`@@unique`/partial index) | App-katmanı tam-1 (`planResponsible`); DB unique YOK; drift-fix scripti mevcut | Mevcut app-layer invariant KORU; DB hardening bu zincirde açma | NEEDS_PRODUCT_DECISION | Migration + mevcut çoklu-responsible veri temizliği gerektirir; erken migration riskli | Veri denetimi (çoklu-responsible kayıt var mı) + migration/temizlik planı | (ileri, ayrı migration gate) | **BLOCKED_PENDING_PRODUCT** |
