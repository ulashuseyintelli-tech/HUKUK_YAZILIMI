...
| `project/docs/governance/receivable-legal-subtype-registry-v1.md` + `receivable-legal-subtype-registry-v1.json` + `.schema.json` + `.checksum.json` + `-crosswalk.md` | Receivable Claim Formation için exact seven-code, versioned Legal Subtype Registry V1 ratification, machine payload, closed schema, deterministic checksum ve Legal Basis/evidence crosswalk pack'i | `RCV-CLAIM-FORM-P02-S08-D02-SR01` RATIFIED / CANONICAL; registry `RCV-CLAIM-LEGAL-SUBTYPE-REGISTRY@1`, checksum `320f671e…c26e64`, runtime DORMANT; provider/resolver/key/signature/signed release/schema/migration authority NONE |
| `project/docs/rcv-claim-legal-basis-projection-binding-v1.md` + `.contract.json` + `.schema.json` + `-field-crosswalk.md` + `-disposition-matrix.md` | Exact Legal Basis release, registry/subtype ve karar projection'ını admission'da canonical payload + checksum ile immutable intent'e bağlayan; finalization'da exact re-resolution ve byte-exact drift kontrolü yapan PB01 contract artefaktları | `RCV-CLAIM-FORM-P02-S08-D02-PB01` CLOSED / CANONICAL / PASS; implementation PR #1794 / `a62e078a`; contract `RCV-CLAIM-LEGAL-BASIS-PROJECTION-BINDING@1`, runtime DORMANT; schema/migration/live apply/key/signature/signed release NONE |
| `project/docs/rcv-claim-legal-signer-*` | Legal Basis için üç role-separated AWS KMS Ed25519 key ceremony, public-key manifest/checksum, custody/access, possession, rotation/revocation/compromise ve redacted audit evidence pack'i | `RCV-CLAIM-FORM-P02-S08-D02-KC01` CLOSED / CANONICAL / PASS; implementation PR #1856 / `74d1950d`. Historical RECEIVABLE task route provenance olarak korunur; KMS/key/trust-root lifecycle owner'ı `CROSS_MODULE / SHARED_CONTROL_PLANE`, signer identity/role eligibility owner'ı `OFFICE`, Legal Basis signature-policy/formation consequence owner'ı `RECEIVABLE`dır. Private custody NON-EXPORTABLE; signing NOT_ACTIVE; production signature NONE; runtime DORMANT |
| `project/docs/rcv-claim-legal-public-key-trust-root-*` + `project/docs/rcv-claim-legal-signature-envelope-verification-contract-v1.md` | KC01'in üç role-separated Ed25519 public key'ini immutable, checksum-pinned verification trust root'a bağlayan closed schema, deterministic validator, exact-version dormant resolver, signature-envelope contract ve redacted AWS parity evidence pack'i | `RCV-CLAIM-FORM-P02-S08-D02-TR01` CLOSED / CANONICAL / PASS; implementation PR #1901 / `3472052b`; trust root `RCV-CLAIM-LEGAL-PUBLIC-KEY-TRUST-ROOT@1`; verification ACTIVE, runtime DORMANT, signing NOT_ACTIVE, production signature NONE. Artefact adı/original route RECEIVABLE-historical; lifecycle/infrastructure owner `CROSS_MODULE / SHARED_CONTROL_PLANE`; signer eligibility owner `OFFICE`; application verification profile consumer ve formation consequence owner `RECEIVABLE`. Yeni workspace module oluşturulmaz |
...
| `project/docs/governance/RCV-PHASE-1-AUTHORIZATION.md` | RCV → CCB-001 identity-only program/register cross-pointer'ı, DEC-0030 disposition'ı, formal phase/workstream/package closure ve owner gate kaydı | CANONICAL / DEC-0030 CLOSED; Receivable Claim Formation için tek living current locator bu belgenin header kaydıdır. Index living status veya execution authority kopyalamaz; current program/phase/workstream/next-task gerçeği için header doğrudan okunur. |
...
| `project/docs/governance/runtime-reconciliation-r01/` (C30–C37 lane kayıtları: `release14-cutover-record-r01.md`, `c30-f01-launcher-pin-atomic-cutover-design-r01.md`, `c32-r04-implementation-rehearsal-qualification-r01.md`, `c33-release18-cutover-record-r01.md`, `c34-r05-production-engine-qualification-r01.md`, `c35-smoke-identity-provisioning-r01.md`, `c36-smoke-principal-engineering-qualification-r01.md`, `c37-external-runtime-storage-application-implementation-r01.md`) | RUNTIME reconciliation lane — RELEASE14 cutover kaydından C37 external runtime storage'a kadar sayfa-başına kanonik kayıtlar; her dosya kendi owner verdict'ini taşır | CANONICAL LANE RECORDS / PRODUCTION AUTHORITY: NONE — merkezi ledger `OFFICE-DELIVERY-MANIFEST.md` §15 (C30–C37 güncel hükümler, evidence SHA-256, residual, supersession); C33 = `RELEASE19_CUTOVER_APPLIED_AND_VERIFIED / TERMINAL_VERIFIED` (2026-09-06; RELEASE18 kaydı `c33-release18-cutover-record-r01.md`), C35 = `SMOKE_IDENTITY_PROVISIONED_AND_AUTHENTICATED_SMOKE_VERIFIED`, C37 owner terminal verdict BEKLİYOR — güncel hüküm ODM §15.8 |
