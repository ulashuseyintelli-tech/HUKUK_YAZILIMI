...
| ADR-014 | CCB-001 Canonical Legal Calculation Core - legacy clean-break cutover constitution | `docs/adr/ADR-014-CCB-001-CANONICAL-LEGAL-CALCULATION-CORE.md` | LOCKED (direction; PR-gated) — **implementasyon branch'i (`codex/ccb-001-pr1-pr6-rescue`) main'e henüz merge edilmedi** | Canonical `computeBalance` / ClaimItem + TBK100 + Interest Engine is the target legal calculation authority; immediate cutover is BLOCKED until PR-1A..PR-9 hardening gates pass. Legacy `getCalculationSummary` is not production authority; no dual-authority production balance, no UI switch or legacy deletion before the locked CCB-001 sequence. Owner-assigned to ADR-014 on 2026-07-10 (branch-local olarak ADR-012 idi; ADR-012/ADR-013 main'de başka hatlara ayrılmış olduğu için GOV-ADR-NAMING-000 ile çakışmayı önlemek için ADR-014'e taşındı). |
...
| ADR-014 | CCB-001 Canonical Legal Calculation Core | LOCKED (direction; PR-gated; branch unmerged) | Owner kararı (2026-07-10). Branch-local eski ismi `ADR-012` idi — main'de o numara DX-005'e ait olduğu için ADR-014'e taşındı. FEE/Harç/Snapshot/Journal için kullanılmaz (o hâlâ ADR-013). |
| CCB-001 | Canonical claim-balance cutover implementation authority / master stream | ACTIVE | Closure/cutover hattı olarak korunur; kendi mimari dokümanı artık **ADR-014**'tür (2026-07-10 owner kararı); FEE implementation hattı değildir. |
