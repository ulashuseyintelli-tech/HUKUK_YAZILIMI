...
| ADR-014 | CCB-001 Canonical Legal Calculation Core — target legal-calculation and application authority constitution | `docs/adr/ADR-014-CCB-001-CANONICAL-LEGAL-CALCULATION-CORE.md` | LOCKED / ALLOCATION-AUTHORITY AMENDED 2026-07-18 / RD01 + XD-001 + TPA-02 RATIFIED 2026-07-19 / TPA-03 OPTION B RATIFIED 2026-07-20 / SHADOW_ONLY / RUNTIME CUTOVER NOT AUTHORIZED | Balance Engine + canonical Receivable snapshot + `LegalCalculationBucket` + TBK100 + Interest Engine target tek hesaplama otoritesidir. ClaimItem legal source/provenance/calculation input'tur; application target/payment-state/allocation authority değildir. Receivable bucket/policy; Collection receipt lifecycle/idempotency/outer transaction orchestration sahibidir. Target persistence, tek logical writer `LegalApplicationWriter` tarafından yalnız canonical Collection transaction'ı içinde yazılan bağımsız `LegalApplicationBatch` aggregate'idir; immutable `LegalApplication` bucket-effect fact'leri ve non-authoritative `ApplicationAttribution` lineage fact'leri taşır. TPA-03 Option B exact two-file additive/writer-free/no-backfill foundation; APPLY/REVERSAL batch ve COST/ANCILLARY/ACCRUED_INTEREST/PRINCIPAL component enum'ları; positive minor-unit amount, tenant-scoped replay, restrictive composite FK ve immutability contract'ını ratifiye eder. Exact-cent canonicaldır, enforcement writer-stage'e bırakılmıştır. Current ClaimItem-keyed `LedgerAllocation` historical legacy record, `CollectionAllocation` canonical-output-derived transitional projection ve `ClaimItem.collectedAmount` frozen legacy cache'tir. ACT-28/REC-AUTH-011/012 OPEN; PR #407 HOLD / DO NOT MERGE / DO NOT REBASE; TPA-03A owner GO-IMPLEMENT required; schema/migration/writer/replay/cutover/retirement NOT AUTHORIZED. `ADR-013` ayrı fee/harç/snapshot owner-review hattıdır. |
...
production call-site, artifact fetch, runtime emission, audit/evidence persistence, external
egress or alert delivery. `REFERENCE_SEALED` verifies only the supplied reference index and chain;
it is explicitly non-official, non-persisted and authority-free. Representative evidence remains
`ABSENT / BLOCKING`; CAN-CUT-02 remains open; PR-11 and runtime cutover remain `NOT AUTHORIZED`.
The pre-evidence observability preparation chain is complete at its authorized boundary. The next
step is `OWNER DECISION REQUIRED — Representative Evidence Preparation`; no preparation or
...
**ADR-014 post-REP-01A-R2 routing override (2026-07-13):** ADR014-REP-01A-R2 is
`CLOSED / CANONICAL / PHASED-CONTRACT-ONLY` after technical PR #1219 (head
`82ccff90f1734a623ef0928dcba6ab40c7ecd1b5`; squash
`3421f5c46b8b32268731fa6d96c72fd290c416b4`) and its governance closure. Version 1 remains
backward-compatible. Version 2 separates owner-backed pre-run decisions from facts knowable only
during or immediately after a separately authorized local run. `PRE_RUN_AUTHORIZED` requires the
environment, operator, reviewer policy, separate access/execution approvals, read-only/no-egress
controls, local output, owner-controlled indefinite retention, manifest/baseline methods and five
sign-offs. Missing session/manifest/reviewer/timestamp/count facts yield
`RUNTIME_BINDING_REQUIRED`; complete binding yields only `CAPTURE_COMPLETE`, never evidence
acceptance, REP-02 authorization, PR-11 readiness or runtime/cutover authority. No package instance,
data access or run occurred. Representative evidence remains `ABSENT / BLOCKING`; CAN-CUT-02 stays
open; PR-11 and runtime cutover remain `NOT AUTHORIZED`. The single next eligible workstream is a
