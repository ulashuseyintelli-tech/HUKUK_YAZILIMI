...
| ADR-014 | CCB-001 Canonical Legal Calculation Core — target legal-calculation and application authority constitution | `docs/adr/ADR-014-CCB-001-CANONICAL-LEGAL-CALCULATION-CORE.md` | LOCKED / ALLOCATION-AUTHORITY AMENDED 2026-07-18 / RD01 + XD-001 RATIFIED 2026-07-19 / SHADOW_ONLY / RUNTIME CUTOVER NOT AUTHORIZED | Balance Engine + canonical Receivable snapshot + `LegalCalculationBucket` + TBK100 + Interest Engine target tek hesaplama otoritesidir. ClaimItem legal source/provenance/calculation input'tur; application target/payment-state/allocation authority değildir. Receivable bucket/policy; Collection receipt lifecycle/execution orchestration sahibidir. Target persistence tek-yazıcı cross-domain boundary'dir; dual authority yasaktır. Physical owner/aggregate seçilmemiştir; `ApplicationBatch` dahil alternatifler TPA-02 read-only analizine bırakılmıştır. Current ClaimItem-keyed `LedgerAllocation` AS-IS/legacy persistence, `CollectionAllocation` compatibility projection only ve `ClaimItem.collectedAmount` no-new-reader/writer deprecated cache'tir. PR #407 HOLD; schema/migration/writer/cutover NOT AUTHORIZED. `ADR-013` ayrı fee/harç/snapshot owner-review hattıdır. |
