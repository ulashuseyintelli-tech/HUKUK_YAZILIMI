...
| ADR-014 | CCB-001 Canonical Legal Calculation Core — target legal-calculation and application authority constitution | `docs/adr/ADR-014-CCB-001-CANONICAL-LEGAL-CALCULATION-CORE.md` | LOCKED / ALLOCATION-AUTHORITY AMENDED 2026-07-18 / SHADOW_ONLY / RUNTIME CUTOVER NOT AUTHORIZED | Balance Engine + canonical Receivable snapshot + `LegalCalculationBucket` + TBK100 + Interest Engine target tek hesaplama otoritesidir. ClaimItem legal source/provenance/calculation input'tur; application target değildir. `LegalApplication` ile `ApplicationAttribution` ayrı fact'lerdir. Current ClaimItem-keyed `LedgerAllocation` AS-IS/legacy persistence, `CollectionAllocation` compatibility projection only ve `ClaimItem.collectedAmount` deprecated/non-authoritative cache'tir. W0.1–W0.3 ve PR-1A..PR-10 tarihsel closure kayıtları korunur; P01/P02 amendment, P03 redesign, target persistence ve schema/migration ayrı owner gate'leridir. PR #407 HOLD / DO NOT MERGE; post-amendment triage required. PR-11, data/replay, consumer switch, primary authority promotion ve runtime cutover NOT AUTHORIZED. `ADR-013` ayrı fee/harç/snapshot owner-review hattıdır. |
