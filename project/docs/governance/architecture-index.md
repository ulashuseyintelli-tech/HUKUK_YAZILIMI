...
| ADR-014 | CCB-001 Canonical Legal Calculation Core — target legal-calculation and application authority constitution | `docs/adr/ADR-014-CCB-001-CANONICAL-LEGAL-CALCULATION-CORE.md` | LOCKED / ALLOCATION-AUTHORITY AMENDED 2026-07-18 / RD01 + XD-001 + TPA-02 RATIFIED 2026-07-19 / SHADOW_ONLY / RUNTIME CUTOVER NOT AUTHORIZED | Balance Engine + canonical Receivable snapshot + `LegalCalculationBucket` + TBK100 + Interest Engine target tek hesaplama otoritesidir. ClaimItem legal source/provenance/calculation input'tur; application target/payment-state/allocation authority değildir. Receivable bucket/policy; Collection receipt lifecycle/idempotency/outer transaction orchestration sahibidir. Target persistence, tek logical writer `LegalApplicationWriter` tarafından yalnız canonical Collection transaction'ı içinde yazılan bağımsız `LegalApplicationBatch` aggregate'idir; immutable `LegalApplication` bucket-effect fact'leri ve non-authoritative `ApplicationAttribution` lineage fact'leri taşır. Exact-cent conservation, tenant-scoped key+hash replay, append-only full reversal, composite tenant FK ve `ON DELETE RESTRICT` zorunludur. Current ClaimItem-keyed `LedgerAllocation` historical legacy record, `CollectionAllocation` canonical-output-derived transitional projection ve `ClaimItem.collectedAmount` frozen legacy cache'tir. ACT-28/REC-AUTH-011/012 OPEN; PR #407 HOLD; schema/migration/writer/replay/cutover/retirement NOT AUTHORIZED. `ADR-013` ayrı fee/harç/snapshot owner-review hattıdır. |
