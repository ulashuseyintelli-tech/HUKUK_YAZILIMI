...
|---|---|---|---|
| F-B01-01 | `GET /api/auth/me` yanıtı `passwordHash`/`tokenVersion`/`passwordChangedAt` içeriyor (yalnız çağıranın kendi satırı) | Credential containment açığı — kaynak zinciri YÜKSEK güven, runtime UNKNOWN | **NEW OWNER DECISION REQUIRED** (PHASE B kapsamına ekleme veya successor) |
| F-B01-02 | Scanner F01 marker'ı `PUBLIC_S0_ONLY` tip-düzeyi string — derlemede silinir; hiçbir dist bu marker'ı içeremez → RELEASE10 için yanlış `STALE` | Ölçüm aracı defekti | **CROSS-LANE FINDING** (X1 teslimatı) — bkz. B01 §7 |
| F-B01-03 | `GET /office/smtp-settings` ve `GET /office/sms-settings` yalnız JwtAuthGuard (PUT karşılıkları F01+ADMIN) — secret'lar maskeli, konfig alanları tenant içi herkese görünür | Yetki asimetrisi (residual) | PHASE B / owner disposition |
| F-B01-04 | `OfficeService.getOrCreate` public ve ham satır döndürür (bugünkü 4 çağıranın tümü yalnız `name` okuyor — sızıntı yok) | Korumasız ham yüzey (residual) | PHASE B / owner disposition |
...
| F-B03-02 | `GET /api/staff/:id` maskesiz **ham TCKN** döndürür; liste ucu maskeler — herhangi bir authenticated tenant kullanıcısı erişebilir | PII ifşası | B04 aday kapsamı (owner) |
| F-B03-03 | Staff mutasyonlarında DTO yok (`body: any`) → global `ValidationPipe(whitelist)` etkisiz | Doğrulama boşluğu | B04 aday kapsamı (owner) |
| F-B03-04 | `POST /api/seed/staff` + `seedAll` → `seed.service.ts:204` `as any` ile StaffService guard'larını bypass eden doğrudan `prisma.staffMember.create` | Seed yüzeyi | **B02 girdisi** (PHASE B) |
