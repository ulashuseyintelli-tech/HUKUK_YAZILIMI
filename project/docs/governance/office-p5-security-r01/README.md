...
| F-B01-04 | `OfficeService.getOrCreate` public ve ham satır döndürür (bugünkü 4 çağıranın tümü yalnız `name` okuyor — sızıntı yok) | Korumasız ham yüzey (residual) | **FIXED — MAIN (2026-09-07)**: modül dışı beş tüketici daraltılmış `getOfficeIdentity()` yüzeyine bağlandı (yalnız `name`); `getOrCreate` modül içinde kalır, cross-module çağrı static guard ile yasak. Kanıt: `office-identity-boundary-fb0104.spec.ts` 5/5 + `office-raw-row-cross-module.static-guard.spec.ts` 3/3. **CANLIDA DEĞİL** — RELEASE20 `08ce8e25` bunu içermez; canlı kabul ayrı. Kayıt: `OFFICE-FB0104-RAW-SURFACE-CONTAINED-R01` |
| F-B01-05 | `Lawyer.uyapToken` şema yorumu "// Şifrelenmiş" — kod karşılığı yok; ayrıca alana yazan hiçbir servis yolu yok ve DB'de dolu satır 0 | Belge/kod tutarsızlığı (düşük) → **öncül BAYAT çıktı: yazma yolu VARDI** | **FIXED — MAIN (2026-09-08)**: “yazan servis yolu yok” öncülü ölçümle çürütüldü — `POST /api/lawyers` gövdesi DTO SINIFI değil satır-içi tip literali ile tipliydi → global `ValidationPipe` metatype `Object` gördüğü için çalışmıyor, gövde `LawyerService.create` içinde `...data` olarak EN SONDA spread ediliyordu. Böylece `uyapToken` (ve `eSignatureSerial` / `uyapUsername`) yazılabiliyor; ayrıca gövdeden gelen `tenantId` güvenilen değeri EZİYOR, `canApproveOfficeActions` K1-4b guardı olmadan set edilebiliyordu. Düzeltme: `CreateLawyerDto` + `LAWYER_CREATE_PERSIST_FIELDS` servis allow-list’i + sunucu denetimli alanların spread SONRASI yazılması + şema yorumunun gerçeğe uydurulması (alan ŞIFRELENMİŞ DEĞİLDİR; şifreleme sözleşmesi kurulmadan DOLDURULMAMALIDIR). Kanıt: `lawyer-create-write-boundary-fb0105.spec.ts` 23/23 + `pure/office-auth-user` 88 suite / 1548 test PASS. **CANLIDA DEĞİL** — RELEASE20 `08ce8e25` bunu içermez. Kayıt: `OFFICE-FB0105-LAWYER-CREATE-WRITE-BOUNDARY-R01`. AÇIK KALAN (owner ürün kararı): `lawyerRank`/`defaultPermissions`/`permissionsLocked`/`canModifyOtherPermissions` create’te H2 otorite kontrolünden GEÇMEZ (update’te geçer) — UI bunları create’te gerçekten ayarladığı için simetriyi kurmak davranış değişikliğidir |
