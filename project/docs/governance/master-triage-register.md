...
| VER-05 | ClaimItem/Due reconciliation kapsamı | Orta | **OPEN / PR-1A + PR-1B + PR-1C IMPLEMENTED / INVENTORY REVIEW PENDING:** PR-0 tenant-scoped, deterministic, `REPEATABLE READ READ ONLY` inventory aracını teslim etti. D1–D2 runtime write contract uygulanmıştır: `originalAmount` creation provenance olarak normal update'lerde korunur; `demandedAmount` canonical ve `amount` aynı-write mirror'dır; `0` geçerlidir ve ilgili read path'lerde falsy fallback değildir. Summary demanded mutation mevcut `CLAIM_ITEM_HIGH_IMPACT_CHANGE` approval hattındadır. D3 ClaimItem→Due reverse-write yasağı static regression ile kilitlenmiş; D4 generic Due update'te `NAFAKA↔non-NAFAKA` iki yönlü sınır geçişi backend mutation öncesinde reddedilmiş ve UI edit seçenekleri aynı sınıra daraltılmıştır. D5 runtime guard'ı FATURA auto-generate yolunu write öncesi kapatır; yeni invoice-derived `TAX_KDV` create ile FATURA `PRINCIPAL→TAX_KDV` geçişi admission ve OfficeApproval executor katmanlarında reddedilir. Tarihsel kayıtlar mutate edilmez, non-FATURA `TAX_KDV` korunur ve Due bridge tek KDV-dahil brüt `PRINCIPAL` otoritesi olarak kalır. **PR-1D schema-migration design gerektirir.** Canlı inventory review, repair, reconciliation/backfill, schema/migration ve cutover yetkilendirilmedi. |
