...
| **OWN-25 (PR #406 stale/conflicting disposition)** | **CLOSED / UNMERGED** (2026-07-11): PR head `53fb852a` güncel `main` karşısında 705 commit geride, GitHub `DIRTY / CONFLICTING` ve salt-okunur merge simülasyonunda `cases/new/page.tsx` content conflict bulundu. Direct merge/rebase/cherry-pick reddedildi. `YOK→YOKSUN`, `AKDI→SABIT` ve unknown→silent `YASAL` eşlemeleri canonical main'e taşınmadı. Korunan current-main rebuild ihtiyaçlarından çok-kalem canlı toplam PR #1092 ile teslim edildi (squash `3d41f4be`, CI 4/4 PASS): aktif create-case Step 5 listesi yalnız commit edilmiş `claimDraftItems[].raw.bakiyeTutar` değerlerini para birimine göre ayrı ve kur dönüşümü yapmadan toplar; backend/schema/migration değişikliği yoktur ve sonuç canonical case balance iddiası taşımaz. Okunur `dues.N.interestType` hatası ile UI faiz değerlerinin API sınırında güvenli normalizasyonu ayrı açık ihtiyaçlar olarak korunur. Sonraki gate: `PR-A — Interest Boundary Contract / GO-ANALYZE`; `YOK` interestType üretmez, `YOKSUN` ayrı hukuki olgudur, `AKDI` contractual provenance'ı korunur, banka/mevduat türleri legacy enum'a kör daraltılmaz ve unknown sessizce `YASAL`a düşmez. |
