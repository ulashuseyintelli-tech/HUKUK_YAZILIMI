...
| CAN-CUT-01 | Due / ClaimItem | `DueModal` + `ClaimItemPanel` aynı case detay sayfasında paralel render (`cases/[id]/page.tsx:63,68,2806,3974`) | `ClaimItem` + `interest-engine` | CUTOVER | pr-a4-0-tool+pr-a4-2-synthetic-evidence-implemented / production-empirical-evidence-absent / exact-mapping-not-frozen / cutover-not-authorized | P1 | Critical | High |
...
- **Disposition:** **OPEN / CUTOVER NOT AUTHORIZED.** VER-05 **OPEN / PR-1A + PR-1B + PR-1C + PR-A2 + PR-A3 + PR-A4-0 + PR-A4-2 IMPLEMENTED / PRODUCTION EMPIRICAL EVIDENCE ABSENT / EXACT MAPPING PENDING** olarak devam eder.
...
- **Implementation disposition:** PR-1A, PR-1B, PR-1C, PR-A2, PR-A3, PR-A4-0 ve PR-A4-2 uygulanmıştır. **PR-A4 blocked:** representative production inventory execution+review, exporter authority kararı, numeric-vs-FAIZT code-space seçimi, deposit short/long semantiği, CONTRACTUAL exact mapping kabulü ve COMMERCIAL_FIXED exact mapping kabulü gerektirir. PR-A5 yetkilendirilmemiştir. PR-1D schema-migration design gerektirir. Generator hardening, precautionary-cost lifecycle, D6 ve cutover bu uygulamaların kapsamı değildir.
- **Required verification:** PR-A4-0 aracı ayrı owner/operasyon yetkisiyle representative production-like veya production dataset üzerinde salt-okunur çalıştırılmalı; sanitize çıktı gözden geçirilmeli ve exporter/rich-code sınıfları owner/hukuk mapping kararına sunulmalıdır. Empty local-development tenant veya PR-A4-2 sentetik fixture execution güvenliğini/classifier dallarını kanıtlar fakat production empirical prevalence üretmez. Koşum otomatik mutation, backfill veya follow-up yetkisi üretmez.
- **Acceptance criteria:** Representative inventory review ve exact mapping/authority kararları tamamlanmadan `Due` write path veya UYAP projection cutover'ı yapılmaz. PR-0, semantic-decision closure, PR-1A runtime contract, PR-A2 write admission, PR-A3 CaseBalance read authority, PR-A4-0 diagnostic tool ve PR-A4-2 synthetic evidence fixture; production empirical doğruluk, reconciliation/backfill, UYAP projection, ReportService/SummaryEngine veya genel balance consumer cutover, schema/migration ya da CAN-CUT-01 cutover yetkisi vermez. Bunların her biri sonuçlara bağlı ayrı owner-authorized workstream gerektirir.
