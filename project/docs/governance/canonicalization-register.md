...
**Son güncelleme:** 2026-07-11 (PR-A2 Interest Boundary write-contract governance closure)
...
| CAN-CUT-01 | Due / ClaimItem | `DueModal` + `ClaimItemPanel` aynı case detay sayfasında paralel render (`cases/[id]/page.tsx:63,68,2806,3974`) | `ClaimItem` + `interest-engine` | CUTOVER | pr-1a+pr-1b-implemented / inventory-review-required / cutover-not-authorized | P1 | Critical | High |
...
- **Disposition:** **OPEN / CUTOVER NOT AUTHORIZED.** VER-05 **OPEN / PR-1A + PR-1B + PR-1C + PR-A2 IMPLEMENTED / INVENTORY REVIEW PENDING** olarak devam eder.
...
- **Implementation disposition:** PR-1A, PR-1B, PR-1C ve PR-A2 uygulanmıştır. PR-A3 ayrı GO-ANALYZE için ready'dir; PR-A4 PR-A3 + exact UYAP mapping kabulüne bağlıdır; PR-A5 yetkilendirilmemiştir. PR-1D schema-migration design gerektirir. Generator hardening, precautionary-cost lifecycle, D6 ve cutover bu uygulamaların kapsamı değildir.
...
- **Acceptance criteria:** Envanter tamamlanmadan `Due` write path kapatılmaz. PR-0, semantic-decision closure, PR-1A runtime contract ve PR-A2 write admission; canlı inventory review, reconciliation/backfill, read authority activation, UYAP projection, schema/migration veya cutover yetkisi vermez. Bunların her biri sonuçlara bağlı ayrı owner-authorized workstream gerektirir.
