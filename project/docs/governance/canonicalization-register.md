...
**Son güncelleme:** 2026-07-11 (VER-05 / CAN-CUT-01 PR-1A Three-Amount Write Contract)
...
| CAN-CUT-01 | Due / ClaimItem | `DueModal` + `ClaimItemPanel` aynı case detay sayfasında paralel render (`cases/[id]/page.tsx:63,68,2806,3974`) | `ClaimItem` + `interest-engine` | CUTOVER | pr-1a-implemented / inventory-review-required / cutover-not-authorized | P1 | Critical | High |
...
- **Disposition:** **OPEN / CUTOVER NOT AUTHORIZED.** VER-05 **OPEN / PR-1A IMPLEMENTED / INVENTORY REVIEW PENDING** olarak devam eder.
...
- **Implementation disposition:** PR-1A uygulanmıştır. PR-1B ayrı GO-IMPLEMENT'e hazırdır. PR-1C kararı kilitlidir fakat implementasyonu ayrıdır. PR-1D schema-migration design gerektirir. Generator batch approval/transaction redesign, precautionary-cost lifecycle ve D3–D6 bu uygulamanın kapsamı değildir.
