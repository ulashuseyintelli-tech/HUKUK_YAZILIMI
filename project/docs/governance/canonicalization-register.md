...
| CAN-CUT-01 | Due / ClaimItem | `DueModal` + `ClaimItemPanel` aynı case detay sayfasında paralel render (`cases/[id]/page.tsx:63,68,2806,3974`) | `ClaimItem` + `interest-engine` | CUTOVER | inventory-tool-implemented / semantics-frozen / inventory-run-required | P1 | Critical | High |
...
- **Disposition:** **OPEN / CUTOVER NOT AUTHORIZED.** VER-05 **OPEN / SEMANTIC DECISIONS FROZEN / INVENTORY TOOL IMPLEMENTED** olarak devam eder.
...
- **Implementation disposition:** PR-1A ve PR-1B ayrı GO-IMPLEMENT'e hazırdır. PR-1C kararı kilitlidir fakat implementasyonu ayrıdır. PR-1D schema-migration design gerektirir. Bu disposition runtime implementasyonu başlatmaz.
...
- **Acceptance criteria:** Envanter tamamlanmadan `Due` write path kapatılmaz. PR-0 ve semantic-decision closure canlı koşum, PR-1 implementasyonu, reconciliation/backfill, schema/migration veya cutover yetkisi vermez; bunların her biri sonuçlara bağlı ayrı owner-authorized workstream gerektirir.
