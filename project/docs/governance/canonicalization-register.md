...
| CAN-CUT-01 | Due / ClaimItem | `DueModal` + `ClaimItemPanel` aynı case detay sayfasında paralel render (`cases/[id]/page.tsx:63,68,2806,3974`) | `ClaimItem` + `interest-engine` | CUTOVER | pr-a4-0-tool+pr-a4-2-synthetic-evidence-implemented / pr-a4-3-owner-legal-crosswalk-frozen / production-empirical-evidence-absent / runtime-not-authorized / cutover-not-authorized | P1 | Critical | High |
...
- **Disposition:** **OPEN / CUTOVER NOT AUTHORIZED.** VER-05 **OPEN / PR-1A + PR-1B + PR-1C + PR-A2 + PR-A3 + PR-A4-0 + PR-A4-2 IMPLEMENTED / PR-A4-3 D1–D9 OWNER-APPROVED / PRODUCTION EMPIRICAL EVIDENCE ABSENT / RUNTIME MAPPING NOT IMPLEMENTED** olarak devam eder.
...
- **Implementation disposition:** PR-1A, PR-1B, PR-1C, PR-A2, PR-A3, PR-A4-0 ve PR-A4-2 uygulanmıştır; PR-A4-3 yalnız governance kararını dondurur. **PR-A4 READY FOR SEPARATE GO-ANALYZE:** bu statü runtime implementasyon yetkisi değildir. PR-A5 yetkilendirilmemiştir. PR-1D schema-migration design gerektirir. Generator hardening, precautionary-cost lifecycle, D6 ve cutover bu uygulamaların kapsamı değildir.
