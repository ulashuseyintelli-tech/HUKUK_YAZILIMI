...
TARAFLAR    : OFFICE 33 — C33/D1 (derleme · Katman 1 · R28 · bu belge ve GO taslağı)
              CLIENT    — İ11 (bağlama + aday doğrulaması #2654 R04 · D3 düzeltmeleri R05 #2657 · R06 #2660 · R07 #2662 ·
                          R-T4a kesin kimlik #2666 @ dd42586c R09; R08 #2665 KAPATILDI)
...
### 1.5 CLIENT İ11 — adaya bağlanmış kimlikler (main @ `dd42586c` = #2666 merge; dosya sha'ları OFFICE 33 ve ana yürütücü tarafından bağımsız ölçüldü)

| | sha256 |
|---|---|
| **İ11 §9 kabul bloğu** (R06; R23'e bağlı) | `27E754A721749443F8B3F2F363B7676989FC115BB6B85E6E0982F62644FC6700` — `CLIENT-LIVE-ACCEPTANCE-I11-R01.md` §9 gömülü bloktan OFFICE 33 yeniden hesapladı (LF, sonda LF); belge `59abb70b..dd42586c` arasında değişmedi |
...
| **D3-P izole R-T4 provası** `scripts\t-rt4-isolated-rehearsal.ps1` (R09; canlı DIŞI, yükseltilmiş) | `2BB3D3E9F7414C234C8318DC4E387DB6F9612FDDC54F927EA908D5C54FF55E2C` — içinde T-KAPA pini `676C1542…` + fonksiyon AST özdeşliği + canlı çağrı satırı denetimi |
...
CLIENT R09 §4 komutu, R09 merge SHA'sındaki prova sha'sına bağlı (ana yürütücü ölçtü: owner komutundaki kapı = main prova sha'sı; prova içindeki
T-KAPA pini = main T-KAPA). Canlı görev/port/`I11-WINDOW-BLOCK-*` kurallarına **dokunmaz**. Yalnız `HYRT4S-<runId>-*` adlı geçici kural ve
...
(sha doğrulamalı). Kanonik çalışma ağacı **`dd42586c`'yi içeren** main'de olmalıdır (2026-09-13 ~12:16Z ölçüldü: HEAD = `dd42586c`; D3-0
...
| D3-1 dış blok `T-AC SHA UYUSMUYOR` ya da `pwsh` bulunamadı | değişiklik 0 (T-AÇ koşmadı) | **D3-S**; neden giderilir (dondurma kuralı / kanonik ağaç `dd42586c`); yeni deneme D3-0'dan |
...
| D3-4 T-KAPA sha uyuşmaz (kanonik ağaç değişmiş) | pencere açık | T-KAPA KOŞULMAZ; betik `dd42586c`'deki blob'dan sha doğrulamalı bayt-kopya ile koşulur — owner kararı; dondurma kuralı ihlali kayda yazılır |
...
>    `7A7228B1143BE2A8406FAF4CA316064EB2E164AE23E160E1353121F64E0EFDDC` · D3-P prova `2BB3D3E9F7414C234C8318DC4E387DB6F9612FDDC54F927EA908D5C54FF55E2C` ·
>    kanonik betik kaynağı main `dd42586ccf07ff4f892c68f964b9e2bddc1fa5d4` (#2666). Geri dönüş: RELEASE22 @ `137406701248858221d12be94a941f8837a2a245`,
...
>    main `dd42586c` + kanonik ağaçta yeniden ölçümü. Sonuçlar kayda yazılmadan sıradaki adıma geçilmez.
