...
| ALC-P0-3 Canonical balance source + cutover tasarımı (B1'in kapanış yolu) | IN-PROGRESS | #857/#861 `interestAccrualStatus`/provenance sözleşmesini ekledi. ALC-P0-3B3 (PR #898, `a8e71a91`) `resolveInterestConfig`'e mixed-source Kademe 1.5 ekledi — 2026/9502 artık canonical (shadow) motor içinde bucket üretiyor. **B1 kapanmış SAYMA**: bu yalnız assembler/orchestration katmanında (`CaseBalanceService`) doğrulandı; canlı UI'nin gösterdiği `case.service.ts` hesabı hâlâ `takipOncesiFaiz=0`/`takipSonrasiFaiz=0` hardcoded olabilir — cutover (canonical'ı PRIMARY display yapmak) ayrı, doğrulanmamış bir adım. 2026/9604-9605 hâlâ eksik veri/pipeline blocker (engine bug değil). ALC-AUTH-3B (PR #917, `8c0cad8f`) `totalDebtAmount` boşluğunu kapattı. ALC-AUTH-3D (PR #922, `8a340c23`, 2026-07-05) frontend guard'ın backend `cutoverReadiness.safeForPrimaryDisplay`/`blockers`'ı tek otorite olarak tükettiğini garanti etti (`HARD_NO_GO_CODES` kaldırıldı) — `OUTSTANDING_DELTA`/`PAID_DELTA`/`PRINCIPAL_BUCKET_DELTA` için authority-source drift KAPANDI. **AMA guarded primary pilot flag HÂLÂ NO-GO, ayrı ve daha temel bir nedenle**: `COSTS_DELTA`/`ATTORNEY_FEE_DELTA`/`EXPENSE_BUCKET_DELTA`/`ATTORNEY_FEE_BUCKET_DELTA` kasıtlı olarak `cutoverReadiness.safeForPrimaryDisplay`'i bloklamıyor (B1_SCOPE_EXEMPT_DIFF_CODES, ALC-AUTH-1A kararı) — cost/ATTORNEY_FEE ClaimItem'ı olmayan bir case'te guard GEÇER, ve `buildGuardedPrimaryCalculationResult()` `toplamBorc`/`sonBorc`/`kalanBorc`'u (`guarded-primary-display.ts:360-363`) canonical `totalDebtAmount`/`outstandingAmount` ile override eder — bu değerler `costs`/`ancillaries` ClaimItem'ı yoksa sessizce 0 sayar (`case-balance-display.ts:483-486`, `sumRecord()` boş kayıt için null değil 0 döner). Sonuç: avukat ekranda gerçek borcun altında bir "TOPLAM BORÇ/SON BORÇ" görebilir (ALC-AUTH-3C'nin 2026/9502 için hesapladığı ~34.311 TL örneği hâlâ GEÇERLİ, ALC-AUTH-3D bunu KAPATMADI). Yeni öneri: **ALC-AUTH-3E** — `buildGuardedPrimaryCalculationResult()`'ı yalnız principal/interest/payment alanlarını (toplamBorc/sonBorc/kalanBorc DAHİL değil ya da cost/vekalet ClaimItem mevcutsa dahil) override edecek şekilde daraltmak (ALC-AUTH-3C'nin (2) numaralı alternatifi) — owner GO-ANALYZE/GO-IMPLEMENT bekliyor, henüz backlog'a işlenmedi. |
