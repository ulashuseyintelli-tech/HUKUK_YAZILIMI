# ═══════════════ İ16 CANLI KABUL (H7 portal) - OWNER TEK BLOK (normal PowerShell; YÖNETİCİ GEREKMEZ) ═══════════════
# YAPAR : kapılar (main senkron/temiz · paket digest · canlı dist · tek API · yabancı yürütücü yok · açık pencere yok) → GO ref
#         YEREL girilir (biçim + tüketilmemişlik) → TEK koşum `i16-live-run.js` (kurulum = İLK YAZMA; H7-00…H7-04, H7-05a,
#         H7-06…H7-08; finally'de personel + PORTAL erişim kapanışı + Case CLOSED; izolasyon) → GO ref tüketim kaydı → manifest.
# YAPMAZ: portal belge YÜKLEME (diske dosya bırakmaz; belge satırları sentetik tenant'ta DB'de kurulur) · forgot-password ·
#         .env/görev/firewall · ikinci API · gerçek alıcıya gönderim.
# SIR   : DB URL canlı .env'den SÜREÇ İÇİNDE okunur, yazdırılmaz; personel ve portal parolaları bellekte üretilir.
# HATA  : kapanış `finally`de koşar; ek güvence (aynı pencerede): `$env:I16_LIVE_CONFIRM='1'; $env:I16_LIVE_GO_REF='x';
#         $env:I16_RECOVER_RUNID='<runId>'; node <scripts>\i16-live-recover.js` (kimlik bağı yoksa SIFIR yazma).
...
$Sc       = Join-Path $Gov 'client-live-acceptance-i16-r01\scripts'
...
function Fail([string]$m) { Write-Host "DUR - $m" -ForegroundColor Red; throw "I16-DUR: $m" }
...
$GoRef = Read-Host 'İ16 canlı GO ref (OWNER-GO-CLIENT-I16-YYYYMMDD-RNN)'
if ($GoRef -cnotmatch '^OWNER-GO-CLIENT-I16-\d{8}-R\d{2}$') { Fail 'GO ref BİÇİMİ hatalı' }
...
$EvDir = Join-Path $EvRoot ("i16-live-{0}-{1}" -f $RunId, (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
$env:I16_LIVE_CONFIRM = '1'; $env:I16_LIVE_GO_REF = $GoRef; $env:I16_RUNID = $RunId
$env:I16_EXPECT_DB = 'hukuk_db'; $env:I16_EXPECT_TENANT_SLUG = "ah-$RunId"; $env:I16_API_BASE = $Api; $env:I16_EXPECT_API = $Api
$rb = New-Object byte[] 18; [Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($rb)   # PS 5.1 uyumlu
$env:I16_LIVE_LOGIN_PW = 'I16L!' + [Convert]::ToBase64String($rb).TrimEnd('=').Replace('+','-').Replace('/','_'); $rb = $null
$env:I16_RECEIPT = Join-Path $EvDir 'i16-setup-receipt.json'; $env:I16_EVID_FILE = Join-Path $EvDir 'i16-evidence.json'
[ordered]@{ record='I16-OWNER-BLOCK'; runId=$RunId; main=$head; packageDigest=$gotPkg; liveDist=$gotDist; apiPid=$lis[0].OwningProcess; startedUtc=(Get-Date).ToUniversalTime().ToString('o') } |
...
& node (Join-Path $Sc 'i16-live-run.js') *> (Join-Path $EvDir 'i16-run.log')
$rc = $LASTEXITCODE
...
[ordered]@{ record='I16-GOREF-CONSUMED'; runId=$RunId; goRefSha256=$goSha; literalWritten=$false; exitCode=$rc; atUtc=(Get-Date).ToUniversalTime().ToString('o') } |
  ConvertTo-Json | Set-Content -LiteralPath (Join-Path $EvDir 'goref-consumed.json') -Encoding UTF8
foreach ($k in 'AH_DATABASE_URL','I16_LIVE_GO_REF','I16_LIVE_LOGIN_PW') { Remove-Item "Env:$k" -ErrorAction SilentlyContinue }
$GoRef = $null
...
Write-Host "İ16 KOŞUM BİTTİ - RUNID=$RunId · çıkış=$rc (0 PASS · 2 FAIL · 3 ÖLÇÜLEMEYEN · 1 DURDU · 5 KAPANIŞ DOĞRULANMADI)" -ForegroundColor $col
