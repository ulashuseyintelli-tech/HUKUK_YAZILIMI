& {
  ...
  if (Test-Path -LiteralPath $SinkLog) {
    ...
    if ($base -lt 0) {
      ...
      if ($newLines.Count -gt 0 -and $withRun.Count -gt 0) {
        $posProof = $true
  ...
  if (-not (Test-Path -LiteralPath $BAK)) { throw "R-T0: on goruntu yedegi YOK ($BAK) - elle mudahale; owner a BILDIR" }
  ...
  if ($curHash -eq $bakHash) {
    ...
    if ($after -ne $bakHash) { throw "R-T1: geri yazma DOGRULANAMADI ($after != $bakHash)" }
  ...
  if ($Mode -eq 'live') {
    ...
    # Beklemeli bicimler ayrilmis torun surec yasadigi surece cagriyi askida birakiyordu.
  ...
  $sw.Stop()
  if ($newPid -eq 0) {
    throw "R-T2: API $BudgetSec sn icinde ayaga KALKMADI ($([int]$sw.Elapsed.TotalSeconds) sn). env ZATEN on goruntudedir. OTOMATIK TEKRAR YOK - owner a DERHAL bildir."
  }
  ...
  if ($proc.StartTime -le $envWrite) { throw "R-T3: API baslangici env yazimindan SONRA DEGIL" }
  ...
  # ---- R-T4: ONLEMEYI KALDIR (yalniz canli) ----
  if ($Mode -eq 'live') {
    ...
    if ($left.Count -ne 0) { throw "R-T4: engelleme kurali kaldirilamadi ($($left.Count))" }
    ...
    $sw2.Stop()
    if ($webUp -ne 1) { throw "R-T4: Web $BudgetSec sn icinde ayaga KALKMADI - owner a DERHAL bildir" }
    Write-Output "R-T4: engelleme kurallari kaldirildi · Web ayakta ($([int]$sw2.Elapsed.TotalSeconds) sn)"
  }

  # ---- R-T5: yakalayiciyi durdur ----
  ...
  if ($still.Count -ne 0) { throw "R-T5: yakalayici hala dinliyor (:$SinkPort)" }
  ...
  if ((Test-Path -LiteralPath $SinkLog) -and (Test-Path -LiteralPath $BaseFile)) {
    ...
    # YALNIZ GERCEK ILETILER sayilir: yakalayicinin 'LISTEN' defter satirlarinda alici YOKTUR
    # ve alan adi da gecmez; onlari yabanci ileti saymak YANLIS-POZITIF uretir (olculdu).
  ...
  Write-Output ''
  Write-Output "PENCERE KAPANDI. Pozitif hedef kaniti: $(if ($posProof) { 'VAR' } else { 'YOK' })"
