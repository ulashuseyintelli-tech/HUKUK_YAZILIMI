...
try {
  ...
  Scen 'S2a kismi hata (2. ad silinemez)' { New-Rec $Names $free; foreach($i in 0..1){ New-IsoRule $Names[$i] $free[$i] }; Stop-TestWeb; $script:rmN=0; function script:Remove-NetFirewallRule { param([string]$Name) $script:rmN++; if($script:rmN -eq 1){ throw 'ENJEKTE (sinama): kaldirma basarisiz' }; NetSecurity\Remove-NetFirewallRule -Name $Name } } { param($l,$e,$p) $e.Count -eq 1 -and ($e[0] -like 'R-T4a: *ENJEKTE*') -and ($l | ? { $_ -like 'R-T4b: Web ayakta*' }) -and $p.iso -eq 1 -and $p.web }
  ...
  Scen 'S5 sorgu hatasi != yokluk' { New-Rec $Names $free; foreach($i in 0..1){ New-IsoRule $Names[$i] $free[$i] }; Stop-TestWeb; function script:Get-NetFirewallRule { param([string]$Name,$DisplayName,$ErrorAction) if($Name -eq $Names[0]){ Write-Error -Message 'ENJEKTE: erisim' -Category PermissionDenied -ErrorAction Stop }; if($DisplayName){ NetSecurity\Get-NetFirewallRule -DisplayName $DisplayName -ErrorAction:$ErrorAction } else { NetSecurity\Get-NetFirewallRule -Name $Name -ErrorAction:$ErrorAction } } } { param($l,$e,$p) $e.Count -eq 1 -and ($e[0] -like '*sorgu/erisim hatasi*yoklukla KARISTIRILMAZ*') -and $p.iso -eq 1 }
