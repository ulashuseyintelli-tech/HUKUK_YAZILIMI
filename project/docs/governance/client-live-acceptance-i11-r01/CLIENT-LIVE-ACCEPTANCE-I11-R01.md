...
| K-PAR | kabul/governance betiği çalıştıran başka süreç 0 (blok kendini imzasıyla dışlar). **Yöntem T penceresinin `smtp-sink-noauth.js`'i dışlanır**; AUTH ilan eden eski `smtp-sink.js` hâlâ durdurur (R05); yakalayıcı kaydı `smtp-sink.jsonl`'u komut satırında taşıyan sarmalayıcı/izleyici durdurmaz (R06) |
...
& {
  ...
  # Sondaki \b yakalayici KAYDINI (`smtp-sink.jsonl`) dislar: komut satirinda log yolu tasiyan
  # sarmalayici/izleyici surec §9'u DURDURMAZ (R06, olculdu); eski `smtp-sink.js` hala durdurur.
