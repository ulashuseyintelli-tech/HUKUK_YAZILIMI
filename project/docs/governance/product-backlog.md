...
- **ALTYAPI GOZLEMI (CI degisikligi GEREKMEZ):** resmi runner `scripts/run-ci-manifest.sh` bu manifesti Windows'ta artik calistiramiyor — 84 yol Windows komut satiri uzunluk sinirini asiyor ("The syntax of the command is incorrect", jest hic baslamadan). Yerel dogrulama ayni secimle gecici `testMatch` config'i ve AYNI runner bayraklariyla (`--ci --forceExit --runInBand`) tek surecte yapildi. Linux CI'da sinir yok; runner DEGISTIRILMEDI.
