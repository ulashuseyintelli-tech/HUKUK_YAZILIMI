...
## ALC-AUTH-6 — Canonical Hesap Özeti Component Coverage / Ground Truth Harness (2026-07-05, GO-ANALYZE)
...
Unlock Condition: Owner GO-IMPLEMENT onayı (ComponentCoverageReport şema + golden-file harness için ayrı, bu kayıtla YETKİLENDİRİLMEYEN bir sonraki adım)
Estimated Size: — (analiz tamamlandı; harness/rapor implementasyonu ayrı, orta-büyük bir GO-IMPLEMENT gerektirir)
Related Modules: case.service.ts (getCalculationSummary, buildCalculationSummaryCanonicalShadow), fee-engine.service.ts, tariff.service.ts, case-balance-display.ts, guarded-primary-display.ts
Status: **GO-ANALYZE DONE — IMPLEMENTATION AYRI, HENÜZ BAŞLAMADI.** Kapsam dışı (bu kayıtla yetkilendirilmez): kod değişikliği yok, schema/migration yok, flag açma/rollout yok, legacy fallback genişletmesi yok, ClaimItem materialization değişikliği yok, payment reconciliation değişikliği yok. **B1 flag varsayılan KAPALI kalmaya devam eder** — flag'in açılması artık yalnız `cutoverReadiness`/ALC-AUTH-3D-3E eşleşmesine değil, component coverage + false-negative riskinin ground-truth'a karşı ölçülmesine de bağlıdır; bu ek ön-koşul burada kayda geçmiştir. Bu, ALC-AUTH-4A'nın (PR #942, MERGED) kapanışını GERİ ALMAZ — 4A kendi dar kapsamında (banner/reasonCodes mismatch) tamdır; ALC-AUTH-6 ondan sonraki, daha derin bir doğrulama katmanıdır.
---
