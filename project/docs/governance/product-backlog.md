...
- **BAGLANMAYAN 24 SPEC (13 tekil + 11'lik pdf-poppler grubu) — kok neden + dar onarim onerisi (urun/fixture/mock/beklenti DEGISTIRILMEDI; hepsi owner karari):**
  ...
  | 9 | `calc-preview/.../object-store/__tests__/object-store.write-once.integration.spec.ts` | kosulmadi (bilincli) | MinIO `health/live` erisilemezse her test `return` ile **vakum PASS** veriyor (skip DEGIL) -> CI'da MinIO yok, baglanirsa sahte yesil; yerelde dev MinIO :9000 acik oldugu icin ona dokunmadim | CI'a MinIO servisi (ci.yml -> governance) VEYA spec'e describe-duzeyi kapi (SPEC); baglama owner karari |
