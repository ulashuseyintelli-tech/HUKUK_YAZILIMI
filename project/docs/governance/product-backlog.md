...
Status: **IMPLEMENTATION AUTHORITY / MASTER STREAM** (owner reconciliation, 2026-07-09; naming clarified by GOV-ADR-NAMING-000 on 2026-07-09, ADR-013 scope further clarified — Option C — on 2026-07-10). CCB-001 is the single implementation-authority workstream for the legacy→canonical claim-balance cutover. `canonicalization-register.md`'s **CAN-CUT-02** (legacy `getCalculationSummary` faiz=0 stub cutover, same `case.service.ts` seam) is a milestone tracked under this workstream, not an independent or competing stream — CAN-CUT-02 remains OPEN/needs-owner-decision in its own register; this does not close it. `ADR-012` is canonical on main only for DX-005 / Waiting & Progress Policy and must not be used as the canonical CCB-001 architecture reference. **Update (2026-07-10, Option C):** the isolated `codex/ccb-001-pr1-pr6-rescue` WIP branch's `ADR-012-CCB-001-CANONICAL-LEGAL-CALCULATION-CORE.md` draft **is** now the basis of the main-canonical `docs/adr/ADR-013-CANONICAL-LEGAL-CALCULATION-CORE.md` (renumbered, scope-clarified, created on main via a dedicated GO-FIX — see decision-log.md 2026-07-10). The canonical target ADR number for this Canonical Legal Calculation Core is **ADR-013**; Fee Projection, Snapshot, Journal, and tariff/statutory-constant/contractual classification are sub-components of ADR-013, not separate ADRs (GOV-ADR-NAMING-000's original "Fee/Harç/Snapshot/Journal" framing was too narrow — corrected). This governance patch creates the ADR-013 file and clarifies its scope; it does **not** authorize PAC-001-A, fee implementation, snapshot/journal spec, or the CCB-001 branch merge itself — those remain separately gated. Branch-local commits `6dfa958d` (CRLF forward-fix) and `0a169f23` (branch-local CCB-001↔CAN-CUT-02↔ADR-012 alignment) remain WIP context on that unmerged branch — they are **not** a global closure and do not by themselves authorize any change here. This entry is the main-synced authoritative record of the CCB-001/CAN-CUT-02/ADR naming relationship.
...
**GOV-ADR-NAMING-000 güncel adlandırma düzeltmesi:** Bu kayıttaki tarihsel "ADR-012'de resmileştirilebilir" ifadesi artık kanonik değildir. `ADR-012` main üzerinde yalnız DX-005 / Waiting & Progress Policy'dir; tarife verisi / kanuni sabit kural / sözleşmesel sınıflandırma için kanonik hedef ADR numarası `ADR-013` olarak ayrılmıştır. **Güncelleme (2026-07-10, Option C):** `ADR-013` dosyası artık oluşturuldu — `docs/adr/ADR-013-CANONICAL-LEGAL-CALCULATION-CORE.md` (Canonical Legal Calculation Core, CCB-001). Tarife verisi / kanuni sabit kural / sözleşmesel sınıflandırma bu ADR'nin bir ALT BİLEŞENİ olarak ele alınacaktır, ayrı bir ADR değil. Bu üçlü sınıflandırmanın ADR-013 içinde ayrı bir bölüm olarak resmileştirilmesi henüz yapılmadı — ayrı owner GO gerekir (bu kayıt onu yetkilendirmez).
