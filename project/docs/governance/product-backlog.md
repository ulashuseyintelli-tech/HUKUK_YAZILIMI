...
- **BAGLANMAYAN 24 SPEC (13 tekil + 11'lik pdf-poppler grubu) — kok neden + dar onarim onerisi (urun/fixture/mock/beklenti DEGISTIRILMEDI; hepsi owner karari):**
  ...
  | 14 | `interest-engine/__tests__/operational.spec.ts` | tek basina 20/20 PASS; 3 onculuyle 98/98 PASS; **tam manifest (469, tek surec) icinde FAIL** ("should calculate interest successfully" beklenen >0, alinan 0) ve tam-26-oncul kumesiyle deterministik yeniden uretildi | **sira-bagimli, cok-dosyali tek-surec etkilesimi**: yarim-kume ikili aramasi `sprint-4.spec` dedi ama `sprint-4`+op tek basina PASS -> suclu tek dosya DEGIL, >=2 oncul birlikte gerekli; jest modul kaydi dosya basina sifirlandigi icin siz kanali gercek `process`/global; kimlik bu turda belirlenemedi (DOGRULANAMADI) | leave-one-out/ddmin ile gerekli oncul kumesini bul (tanilama, ~26 kosum); bulunan sizinti (env/global) spec'te geri alinmali (SPEC) — sonra `platform-scripts-shared`'a bagla |
