...
- **Onarim (en kucuk):** yeni kok `src/scripts/adr014-shadow-evidence-runner.module.ts` = `AppModule`'deki dort global kaydin birebir yansimasi + `BalanceDisplayShadowDiffModule`; runner bu koku kullanir. Urun modulleri DEGISMEDI.
...
- **Izole gercek kosum (disposable `postgres:16-alpine`, tmpfs, yalniz 127.0.0.1, DB `hukuk_adr014_gate_test`, 131 migration; 3 materializer vakasi: tek odeme / odeme+iptal / odemesiz):** `Read-only dogrulandi`; 3/3 `SUCCESS` (legacy + canonical kaynak mevcut); 5 kanit dosyasi yazildi, `manifest.sha256` icindeki 4 hash bagimsiz hesaplananla BIREBIR; `readOnlyVerified=true`; DB yazma sayaci (`pg_stat_user_tables` ins+upd+del) kosum oncesi/sonrasi **esit**. Uc vakanin karari `DISCREPANCY` (`NON_ZERO_FINANCIAL_DELTA`) — sentetik vakada legacy/canonical farki, runner kusuru DEGIL; yorumlanmadi.
...
- **Kilit:** `scripts/__tests__/adr014-shadow-evidence-runner.module.wiring.spec.ts` (gercek grafik `compile()`, override YOK; dort global + ScheduleModule yoklugu; options kacisi) → `pure/platform-scripts-shared` 487→488. Mutasyon kontrolu: dort global cikarilinca spec ayni `RangeError` ile FAIL.
- **ACIK KALAN (bu onarimin DISINDA, owner GO'su geregi eklenmedi):** runner kanit yazip DB oturumlarini kapattiktan sonra surec kendiliginden CIKMIYOR; JS duzeyinde gorunen aktif kaynak yalniz stdout/stderr borulari (`PipeWrap:2`). Kanit ciktisini etkilemez; kok neden forceExit acik-handle maddesiyle ayni siniftadir ve o maddede ele alinir. Runbook bunu "bilinen acik" olarak yazar.
