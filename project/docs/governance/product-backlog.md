...
| **CLF-O0-01** — genel onay kutusunda `request-revision` PR-1.3 domain-sahiplik kapisi (X4 successor kaydi) | **MAIN`DE TAMAM / CANLIDA DEGIL** — #2608 | OFFICE (+CLIENT FD) | FD talebi (`CLIENT_FINANCIAL_DISCLOSURE_APPROVE`) genel kutunun DORT karar rotasindan da karar almaz → 409 `DOMAIN_ACTION_REQUIRED`; kapi talep okunduktan hemen sonra, durum / oz-onay / rol / yetki denetiminden ve her yazmadan ONCE (reject ile ayni sira). Kapi rol denetiminden once geldigi icin AK-1a (ek) satirindaki "FD talepleri dahil → 403" genel kutu icin artik 409'dur; VIEWER siniri FD disi turlerde ve FD domain yollarinda (ofis / icerik onayi) DEGISMEDI. Diger turlerin karar davranisi DEGISMEDI; gecmis REVISION_REQUESTED kayitlari DEGISTIRILMEDI ve canlida OLCULMEDI. Kaynak kayit `office-x4-r01/clf-o0-01-successor-record-r01.md`; tarihsel P8-C4 envanter satiri DEGISTIRILMEDI. Owner GO 2026-09-10 (task-bound). Canli yayin + kabul ayri asama. |
...
sozlesmesi/intake vb.) rol elemez — bu kayitta envanterlenmedi · genel kutuda `cancel` (talep sahibinin geri cekmesi)
PR-1.3 kapisini cagirmaz: FD talebi genel kutudan CANCELLED'a cekilebilir ve surum ayni kilide duser (CLF-O0-01 kapsami DISI; FD'de domain ici geri cekme yolu yok — owner karari)
