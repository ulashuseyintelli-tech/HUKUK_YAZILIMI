...
- **Baslangic kablolamasi kaniti (davranis testinden AYRI):** `main.ts`'i okuyan mevcut bir mimari test mekanizmasi YOK → **kaynak incelemesi**: `main.ts` `applyTrustProxy`'yi import eder ve `NestFactory.create`'ten hemen sonra, CORS'tan once cagirir; baska `trust proxy` ayari yok. `tsc --noEmit` (main.ts + yapilandirma + spec) hatasiz.
