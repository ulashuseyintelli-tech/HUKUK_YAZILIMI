...
Current W0.2 supersession/remediation state (2026-07-10): **PR #1050 AUTHORIZATION_AND_TECHNICAL_SCOPE_VIOLATION / NARROW REMEDIATION IN PROGRESS / NOT CLOSED.** PR #1050 implemented and merged W0.2 without active owner authorization and contained out-of-scope direct REVERSAL behavior. Owner decision v1.3 narrows W0.2 to declarative normal PAYMENT setup only. The active remediation removes reversal input/output contracts, direct reversal writes, `reversesLedgerEntryId`, derived negative amounts and reversal-specific tests while preserving test-only PAYMENT setup. This record does not retroactively authorize PR #1050. W0.2 remains open until disposable-DB validation, PR-specific CI execution, remediation merge and repository-based Master Register closure complete. W0.3, PR-1A, PR-1B and CCB runtime cutover remain blocked.
Unlock Condition: Bu owner arbitration kaydı yalnız tasarım kararını sabitler. W0.2 REVERSAL scope v1.3 ile **OUT OF SCOPE** olarak daraltılmıştır; W0.2 yalnız declarative normal PAYMENT setup destekler. W0.2/W0.3/PR-1A/PR-1B yalnız kendi güncel gate'leri ve ayrı owner GO ile ilerler. PR #1050 remediation validation ve repository-based Master Register closure tamamlanmadan W0.2 CLOSED sayılamaz.
...
Unlock Condition: Bu owner-approved baseline yalnız yürütme sırasını ve merge kriterlerini sabitler. Implementation AYRI GO-IMPLEMENT'ler gerektirir; sıradaki hat **ADR-014 Wave 0 Implementation** (REVERSAL kararı RESOLVED/Conditional B ile açıldı).
...
Current Plan Status: **APPROVED / BASELINE EXECUTION PLAN v1.3 / DIRECT_RESCUE_MERGE_NO_GO / RESCUE_SOURCE_ONLY.** v1.3 supersedes only the W0.2 direct-reversal provisions of v1.1/v1.2; historical records remain below. W0.2 is PAYMENT-only, PR #1050 remediation is in progress, and PR-1A is an owner-review gate with no implementation authorization.
Status: **APPROVED / BASELINE EXECUTION PLAN v1.2 / DIRECT_RESCUE_MERGE_NO_GO / RESCUE_SOURCE_ONLY (2026-07-10)**. Kanonik belge: `project/docs/design/adr-014-split-pr-plan.md`. **v1.1 delta:** REVERSAL owner arbitration (CONDITIONAL OPTION B) — §11 çözüldü, PR-1B acceptance gate'ine gerçek `CollectionService.cancel()` DB integration eklendi. **v1.2 delta:** W0.2 iskele owner arbitration (Hard Stop → RESOLVED BY OWNER DECISION) — iskele katmanı materializer içinde Prisma-direct, G1–G6 bağlayıcı acceptance gate'leri (G3 = scoped before/after delta==0, global count DEĞİL); Conditional Option B'nin diğer tüm şartları ve `Materializer PASS ≠ cancel() PASS` guardrail'i değişmedi. Kritik terminoloji: W0 implementation prerequisite DEĞİL, DB-gated verification capability enabler'ıdır; Done ≠ Mergeable; ADR-014 bir Scenario Platform istemiyor; branch `961bbaf3` wholesale merge edilmeyecek ve yalnız rescue/evidence source olarak kullanılacak; REVERSAL = **RESOLVED / CONDITIONAL OPTION B**. Runtime cutover PR-1A..PR-9 gates revalidated + DB-gated evidence PASS olmadan başlamaz; PR-10..PR-14 later-stage only kalır. Owner Gate REQUIRED = PR-11/PR-12/PR-14 (production-görünür/geri-dönüşü-zor). Açık owner kararları/bloklayıcılar: duplicate TBK100 disposition, PR-7 fee projection boundary with ADR-013, legal signoff refresh policy; REVERSAL production method artık v1.1/Conditional Option B ile resolved, fakat PR-1B gerçek `CollectionService.cancel()` DB integration gate'i blocker olarak kalır. Revizyon v2/v3 olarak işlenir. Bu kayıt kod/test/schema/migration/runtime değişikliği yaratmaz, scenario infra implementation'ı veya runtime cutover'ı yetkilendirmez.
