...
| ALC-AUTH-3D | Alacak Kalemi | Guard authority alignment (frontend consumes backend cutoverReadiness.safeForPrimaryDisplay) | PR #922 squash merged, SHA `8a340c23`; HARD_NO_GO_CODES/issueCodes()/NOT_COMPARABLE removed, domain-safety single-sourced from cutoverReadiness; CI 4/4 PASS; balance-shadow-display.test.tsx 76/76 PASS; closes authority-source drift for OUTSTANDING_DELTA/PAID_DELTA/PRINCIPAL_BUCKET_DELTA only — does NOT close the separate cost/attorney-fee DATA_GAP understatement risk, see ALC-AUTH-3E; guarded pilot flag still OFF by default |
...
Related Modules: guarded-primary-display.ts, balance-display-shadow-diff.service.ts
Status: **MERGED/CLOSED** — (3) onerilen secenek implement edildi ve merge edildi (2026-07-05). PR #922, commit SHA `a3a7872b`, squash merge SHA `8a340c23`. `guarded-primary-display.ts`'te `HARD_NO_GO_CODES`/`issueCodes()`/`NOT_COMPARABLE` kaldirildi; domain-safety artik dogrudan `report.cutoverReadiness.safeForPrimaryDisplay`/`blockers`'tan okunuyor. Frontend'de yalniz genuine kaygilar kaldi (feature-flag/scenario/payment-designation/periodic-obligation policy + `canonicalPrimaryAmounts()` render-veri-mevcudiyeti). CI 4/4 PASS (Architectural Guardrails, Test Suite, Web Tests (vitest), Client Workspace Live Smoke); `balance-shadow-display.test.tsx` 76/76 PASS; `tsc --noEmit` temiz. 2 test fixture'i (currency/context mismatch, overpayment-blocked) `cutoverReadiness`'i comparability/diagnostics ile tutarli doldurmak icin guncellendi. Allocation-engine/Q5/schema/case.service.ts legacy display'e dokunulmadi. **ÖNEMLİ SINIRLILIK (Master Register kapanış-kontrolü sırasında bulundu, 2026-07-05):** bu kayıt yalnız `OUTSTANDING_DELTA`/`PAID_DELTA`/`PRINCIPAL_BUCKET_DELTA` için authority-source drift'i kapatır. `COSTS_DELTA`/`ATTORNEY_FEE_DELTA`/`EXPENSE_BUCKET_DELTA`/`ATTORNEY_FEE_BUCKET_DELTA` kasıtlı olarak `cutoverReadiness.safeForPrimaryDisplay`'i bloklamadığından (B1_SCOPE_EXEMPT_DIFF_CODES), cost/attorney-fee ClaimItem'ı olmayan bir case'te guard GEÇER ve `buildGuardedPrimaryCalculationResult()` `toplamBorc`/`sonBorc`/`kalanBorc`'u canonical `totalDebtAmount`/`outstandingAmount` ile override eder — bu değerler eksik ClaimItem'ları sessizce 0 sayar (`sumRecord()`, null değil). ALC-AUTH-3C'nin ~34.311 TL under-display riski **HÂLÂ AÇIK**, bu PR'la kapanmadı — bkz. yeni **ALC-AUTH-3E** kaydı. **Guarded primary pilot flag hala varsayilan KAPALI** ve ALC-AUTH-3E kapanmadan AÇILMAMALI.
...
Business Value: Guarded primary pilot flag'in gerçekten güvenle açılabilmesi için son, somut, ölçülmüş blocker'ın kapatılması — flag açılmadan önce avukatın gördüğü toplam borcun asla gerçek borcun altında olmaması garantisi.
Technical Value: ALC-AUTH-3C'nin (2) numaralı alternatifi: `buildGuardedPrimaryCalculationResult()`'ı yeniden kapsamla — ya (a) `toplamBorc`/`sonBorc`/`kalanBorc`'u override listesinden çıkarıp legacy'de bırak (cost/vekalet DATA_GAP olduğu sürece bu aggregate'ler de legacy kalır — ama o zaman "canonical primary" iddiası aggregate seviyesinde eksik kalır), ya da (b) cost/attorney-fee ClaimItem mevcudiyetini ayrı bir sinyal olarak backend'den taşı (örn. yeni bir `costCoverageComplete` alanı) ve yalnız bu sinyal true ise toplamBorc/sonBorc/kalanBorc override et, aksi halde legacy'de bırak. (b) daha doğru ama backend contract değişikliği gerektirir (yeni alan); (a) daha dar/hızlı ama "canonical primary" iken aggregate'lerin legacy kalması semantik olarak tuhaf olabilir — owner kararı gerekir.
Priority: —
...
Unlock Condition: Owner karari — (a) aggregate'leri override listesinden çıkar (XS, hızlı) mı, (b) backend'den cost-coverage sinyali taşı (S-M, daha doğru) mı?
Estimated Size: (a) XS, (b) S-M
Related Modules: guarded-primary-display.ts, case-balance-display.ts, case-balance.service.ts
Status: BACKLOG — kod değişikliği YAPILMADI. **Guarded primary pilot flag bu madde kapanmadan ACILMAMALI** (ALC-AUTH-3D'nin kapattığı authority-source drift'ten bağımsız, ayrı ve daha temel bir risk).
---
