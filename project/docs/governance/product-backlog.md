...
- **BAGLANMAYAN 24 SPEC (13 tekil + 11'lik pdf-poppler grubu) — kok neden + dar onarim onerisi (urun/fixture/mock/beklenti DEGISTIRILMEDI; hepsi owner karari):**
  ...
  |---|---|---|---|---|
  | 1 | `debtor-scoring/__tests__/debtor-scoring.module-registration.spec.ts` | DB env tanimsiz: FAIL 0/1 | Nest DI: `RateSyncService` 3. bagimliligi `IntegrationErrorReporter` **InterestEngineModule** baglaminda cozulemiyor (urun modul kablolamasi eksik/tasinmis) | `IntegrationErrorReporter`'i saglayan modulu `InterestEngineModule` importlarina ekle (URUN, tek satir); spec degismez |
  | 2 | `interest-engine/__tests__/interest-engine.module.wiring.spec.ts` | DB env tanimsiz: FAIL 0/1 | #1 ile AYNI kok neden (ayni hata metni) | #1 ile birlikte kapanir |
  | 3 | `summary-engine/__tests__/summary-engine.module.wiring.spec.ts` | DB env tanimsiz: FAIL 0/1 | `GazetteWatcherService` 1. bagimliligi `IntegrationErrorReporter` **TariffModule** baglaminda cozulemiyor | ayni saglayici modulu `TariffModule`'e ekle (URUN); #1-#3 tek dar PR |
