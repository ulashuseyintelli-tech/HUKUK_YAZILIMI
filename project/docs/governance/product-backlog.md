...
| OFFICE-AUTH-P01 | OFFICE / Auth-Session | Parola görünürlüğü, gerçek "beni hatırla", self-servis parola değişimi; ek olarak `User.tokenVersion`/`passwordChangedAt` (additive migration) ile parola-değişimi tetiklemeli JWT/session revocation temeli | **PARTIALLY DELIVERED / OUT-OF-BAND (retrospective reconciliation, owner arbitration R2, 2026-07-21):** PR #1466 squash merge `468ddb9bd99d682b34a876ad3d7c32365c702762` main'de (2026-07-20). CI 4/4 PASS, 39 yeni test (7+9 unit auth/user + 3 integration + 20 web) + 47 mevcut auth/invite regresyonu + 1123 web testi yeşil. Bu iş, `OFFICE-DELIVERY-MANIFEST.md`'deki CANDIDATE-B (JWT/Session Revocation Mechanism, OFF/OD-15'e bağlı, WAVE 1 decomposition) için ratifiye edilmiş GO-ANALYZE→Contract Draft→Ratification→GO-IMPLEMENT zincirinden GEÇMEDEN, ayrı/adlandırılmamış bir hat olarak merge edildi — governance sequence bypass olarak kaydedilir. Teslim edilen: `User.tokenVersion`+`passwordChangedAt` schema/migration · `JwtPayload.tokenVersion` taşınması · `AuthService.validateUser()` her istekte tokenVersion doğrulaması · `changeOwnPassword` sonrası tokenVersion increment (tek tetikleyici). Teslim edilmeyen/açık kalan: offboarding/staff-lawyer deaktivasyon-tetiklemeli revocation · diğer olası tetikleyiciler · kısa access-token TTL kararı+uygulaması (`JWT_EXPIRES_IN` hâlâ varsayılan `7d`) · refresh-token lifecycle/revocation tasarımı — hepsi **DEFERRED / OPEN / OWNER-GATED FUTURE SCOPE**. Detaylı reconciliation: `decision-log.md` § 2026-07-21 OFFICE-AUTH-P01-RETROSPECTIVE-RECONCILIATION-R2; `OFFICE-DELIVERY-MANIFEST.md` §4/§4b CANDIDATE-B satırı; `OFFICE-RISK-REGISTER.md` STF-PRD-SES-002 kartı. Ayrıca bkz. `OFFICE-AUTH-P02` (PR #1476, credential-recovery mimari kararı — ayrı, bağımsız iş, bu satırla birleştirilmez). **CANDIDATE-B ownerSelectionStatus/contractStatus/implementationAuthorization bu kayıtla DEĞİŞMEDİ (NOT_SELECTED/NOT_DRAFTED/NONE korunur) — kodun mevcudiyeti izlenmemiş governance zincirini geriye dönük olarak gerçekleşmiş SAYMAZ (SYS-DEC-008).** |
| OFFICE-AUTH-P01B | OFFICE / Auth-Session | Login sayfası: "beni hatırla" alanlarının hatırlanması + zaten authenticated kullanıcının otomatik yönlendirilmesi — OFFICE-AUTH-P01'in devamı, ayrı hat | **MERGED (retrospective register kaydı, 2026-07-21):** PR #1472 "feat(auth): remember login fields and auto-redirect authenticated users" main'e merge edildi (commit `4c15b469`). Diff kapsamı yalnız `login/page.tsx` + `auth-context.tsx` + testler (web/client-side, schema/migration/API YOK; parola hiçbir zaman saklanmaz). Doğrulanmış sonuç: PR #1472 CANDIDATE-B'nin kalan session-revocation kapsamına (offboarding-tetiklemeli revocation, kısa TTL, refresh-token) DOKUNMADI — bu kapsam DEFERRED/OWNER-GATED FUTURE SCOPE olarak açık kalır. Bu satır yalnız discoverability için OFFICE-AUTH-P01'e bağlı bir Master Register pointer'ıdır. |
