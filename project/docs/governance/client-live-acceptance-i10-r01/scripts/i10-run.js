/*
 * CLIENT İ10 — TEK YÜRÜTÜCÜ (H3 vekâlet kabulü)
 *
 * GERÇEK YÜRÜTME SIRASI:
 *   1) i10-01-setup     → 7 satır COMMIT (Tenant · User×3 · Lawyer(PARTNER) · Client×2)
 *   2) i10-02-poa       → P-0v/u/e/x · A-1 · A-3 · A-2 · A-4 · K9 · P-K9 (ölçüm geçerliliği)
 ...
 *   6) İZOLASYON        → İ9 paketinin i9-03-isolation.js'i (KOPYA DEĞİL): komşu tenant özeti
 *
 * KURALLAR:
 ...
 *   · Yetkisiz yazma dur kuralı i10-02'dedir (403 dışı / istek hatası / yazma izi → sonraki yetkisiz
 *     deneme gönderilmez).
 *   · Parola yalnız bellekte; alt sürece yalnız ortam değişkeni olarak geçer; yazılmaz.
 *   · Login bütçesi: 3 (aktörler) + 1 (doğrulama) = 4 < 10/dk (LoginRateLimitGuard, IP başına).
 *     429 gelirse ilgili iddia ÖLÇÜLEMEDİ olur — 429 kanıt SAYILMAZ.
 *   · Kütüphane yolları (CL_PRISMA_ROOT, CL_BCRYPT_PATH) ZORUNLU; varsayılana düşülmez.
 *   · ÖN KONTROL (G-0'dan ve her yazmadan ÖNCE): canlıda GO ref'i YALNIZ `OWNER-GO-CLIENT-I10-…`
 *     olabilir — başka işin ref'i (tüketilmiş İ9 GO'su dahil) koşumu açamaz; i10-01 ve i10-02 aynı
 *     denetimi ayrıca yapar. A-4 dosya ayağının kökleri (CL_POA_UPLOAD_ROOT) i10-02'nin AYNI ölçüm
 *     fonksiyonuyla listelenebilir olmalı; değilse koşum başlamaz.
 *
 * ZORUNLU: CL_ENVIRONMENT · CL_DATABASE_URL · CL_API_BASE_URL · CL_STATE_FILE · CL_PRISMA_ROOT ·
 *          CL_BCRYPT_PATH · CL_POA_UPLOAD_ROOT (A-4 dosya ayağı; ';' ile ayrılmış kök listesi)
...
const POA = require('./i10-02-poa'); // yalnız dosya ayağı ölçüm fonksiyonu (yan etkisiz)
...
const SECRET = process.env.CL_LOGIN_PASSWORD || `I10-${crypto.randomBytes(18).toString('base64url')}!aB9`;
...
(async () => {
  ...
  try {
    for (const k of ['CL_PRISMA_ROOT', 'CL_BCRYPT_PATH', 'CL_POA_UPLOAD_ROOT']) {
      if (!process.env[k]) throw new Error(`${k} ZORUNLU — varsayilana SESSIZCE dusulmez; hicbir yazma yapilmadi`);
    }
    POA.assertI10GoRef(); // canlıda YALNIZ İ10 GO ref'i — G-0'dan ve her yazmadan ÖNCE
    const roots = process.env.CL_POA_UPLOAD_ROOT.split(';').map((s) => s.trim()).filter(Boolean);
    const pre = POA.uploadDirState('i10-on-kontrol', roots);
    if (!pre.measurable) throw new Error(`A-4 dosya ayagi OLCULEMEZ (${pre.why}) — hicbir yazma yapilmadi`);
    console.log(`[ON KONTROL] A-4 dosya ayagi olculebilir: ${roots.length} kok · ${POA.rootsDesc(pre)}`);
    env = L.assertEnvironment(); // G-0
    console.log(`\n=== CLIENT I10 KOSUMU · runId=${RUN_ID} · ortam=${env.environment}`
    ...
    const setup = run(path.join(HERE, 'i10-01-setup.js'));
    if (setup.code !== 0 && setup.code !== 4) failure = `kurulum basarisiz (exit ${setup.code})`;
    else setupOk = true;

    if (setupOk) {
      const m = run(path.join(HERE, 'i10-02-poa.js'));
      measureRecord = m.record;
      if (m.code === 1) failure = failure || 'vekalet olcumlerinde FAIL var';
      else if (m.code === 3) failure = failure || 'vekalet olcumlerinde OLCULEMEYEN var';
      else if (m.code !== 0) failure = failure || `vekalet olcumu beklenmeyen cikis (exit ${m.code})`;
    }
  } catch (e) {
    ...
    if (measureRecord) {
      ...
      console.log(`    yetkisiz yazma denemeleri: cagrilan=${calls.join(',') || '-'}${stop ? ` · DURDU: ${stop}` : ' · dur kurali tetiklenmedi'}`);
    }
    ...
    console.log(JSON.stringify({
      record: 'CL-I10-RUN', runId: RUN_ID, environment: env ? env.environment : null,
      ...
      measurements: measureRecord
        ...
        : null,
      poaId: measureRecord ? measureRecord.poaId : null,
