...
| COL-RISK-G07 | RECEIVABLE–COLLECTION legal-application persistence ve cutover zinciri tamamlanmadı | XD-001 authority sınırını; TPA-02 Option D ise independent `LegalApplicationBatch`, immutable `LegalApplication`, non-authoritative `ApplicationAttribution`, single writer/transaction, exact-cent, replay, reversal ve tenant-safe persistence kontratını canonical hale getirir. ClaimItem target/payment-state/allocation authority değildir. `ClaimItem.collectedAmount` frozen legacy cache; `CollectionAllocation` canonical-output-derived transitional projection; `LedgerAllocation` historical legacy record'dur. Schema/migration, writer, representative replay/evidence, consumer cutover ve retirement henüz yoktur. | **OPEN — PHYSICAL MODEL DECIDED / IMPLEMENTATION + CUTOVER + RETIREMENT REMAIN** | ACT-28 / REC-AUTH-011/012 OPEN; `codex/rcv-ws04-p03-syn-01` disposition + PR #407 HOLD + deterministic bucket identity + representative evidence + cutover authority blocker; TPA-03 GO-ANALYZE OWNER GATE; implementation authority NONE |
