...
| COL-RISK-G03 | External-settlement gate ve unapplied payment lifecycle'ı runtime'da yok (overpayment ile örtük eşitlik riski) | COL/OD-06 Option A candidate-first/orthogonal contract RECORDED. W2.2A PR #1332 / `88290071`: exact enum + nullable/defaultsuz candidate status, no backfill, legacy `NULL` unknown/not-SETTLED. W2.2B PR #1347 / `61b49ce0`: yeni `INCOMING` receipt `PENDING`, `OUTGOING` lifecycle dışı; tenant-scoped detection-only `tryAutoMatch`; duplicate sync/backfill yok ve Collection/journal/event/outbox/ledger/allocation/overpayment etkisi sıfır. `SETTLED`/`REJECTED`, settlement evidence, canonical Collection confirmation ve application lifecycle hâlâ yok. | **OPEN — FULL RUNTIME LIFECYCLE INCOMPLETE** | W2.2A CLOSED / CANONICAL; W2.2B CLOSED / CANONICAL upon approved reconciliation merge; W2.2C owner GO bekler; W2.3 `BLOCKED — W2.2 BOUNDARY PENDING` |
