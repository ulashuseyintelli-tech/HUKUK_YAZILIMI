...
| COL-RISK-G07 | RECEIVABLE–COLLECTION legal-application persistence ve cutover zinciri tamamlanmadı | XD-001 authority sınırını; TPA-02 Option D independent aggregate'i; TPA-03 Option B contract'ı canonical hale getirdi. TPA-03A PR #1449 / `63f0b0ea`, exact `schema.prisma` + tek migration scope'unda `LegalApplicationBatch`, immutable `LegalApplication` ve non-authoritative `ApplicationAttribution` additive foundation'ını; tenant-safe restrictive FK, replay/reversal/nonblank-bucket/minor-unit row guards ve immutable UPDATE/DELETE trigger'larıyla kurdu. Foundation writer-free/no-backfill; runtime/test/consumer etkisi yoktur. ClaimItem target/payment-state/allocation authority değildir. `ClaimItem.collectedAmount` frozen legacy cache; `CollectionAllocation` canonical-output-derived transitional projection; `LedgerAllocation` historical legacy record'dur. Writer, aggregate conservation enforcement, representative replay/evidence, consumer cutover ve retirement hâlâ yoktur. | **OPEN — PHYSICAL MODEL + SCHEMA FOUNDATION CLOSED / WRITER + EVIDENCE + CUTOVER + RETIREMENT REMAIN** | ACT-28 / REC-AUTH-011/012 OPEN; `codex/rcv-ws04-p03-syn-01` schema foundation için artık blocker değil, writer/evidence/cutover için BLOCKING; PR #407 HOLD / CONFLICTING / DO NOT MERGE / DO NOT REBASE / UNTOUCHED; next TPA-04 writer-contract analysis OWNER GO-ANALYZE REQUIRED |
