...
| COL-RISK-G03 | External-settlement gate ve unapplied payment lifecycle'ı runtime'da yok (overpayment ile örtük eşitlik riski) | COL/OD-06 Option A candidate-first/orthogonal contract RECORDED. W2.2A PR #1332 / squash `88290071`: exact `PENDING \| SETTLED \| REJECTED` enum + nullable/defaultsuz `BankTransaction.candidateStatus`; no backfill, legacy `NULL` unknown/not-SETTLED. Runtime writer ile external-finality/application eksenleri hâlâ yok. | **OPEN — CONTRACT DECIDED / ADDITIVE SCHEMA FOUNDATION PRESENT / RUNTIME LIFECYCLE ABSENT** | W2.2A CLOSED / CANONICAL upon approved reconciliation merge; W2.2B owner GO bekler; W2.3 `BLOCKED — W2.2 BOUNDARY PENDING` |
