...
| COL-RISK-G07 | RECEIVABLE–COLLECTION legal-application persistence ve cutover zinciri tamamlanmadı | XD-001 boundary, TPA-02 aggregate, TPA-03 contract/03A foundation, TPA-04 writer contract ve TPA-04A snapshot/bucket identity canonicaldır. TPA-04B PR #1470 / `9dabe8db`, required/default-free/no-backfill canonical TEXT evidence alanlarını, per-batch bucket uniqueness ve arithmetic guard'larını, immutable protection'ı, nonempty-foundation hard stop'u ve `receiptAmountMinor = SUM(appliedAmountMinor) + heldRemainderMinor` conservation'ını exact iki dosyada kurmuştur. PostgreSQL 16 apply/rollback/re-apply PASS; runtime writer, backfill ve live DB apply yoktur. Plan builder, dormant/reversal writer, representative evidence, consumer cutover ve retirement hâlâ yoktur. | **OPEN — PHYSICAL MODEL + FOUNDATION + WRITER/SNAPSHOT/EVIDENCE CONTRACTS + SCHEMA AMENDMENT CLOSED / PLAN + WRITER + EVIDENCE + CUTOVER + RETIREMENT REMAIN** | ACT-28 / REC-AUTH-011/012 OPEN; synthetic corpus schema amendment için non-blocking, TPA-04C writer/evidence/cutover için BLOCKING; PR #407 CLOSED/UNMERGED/NO FURTHER ACTION; next `TPA-04C` OWNER GO-ANALYZE REQUIRED / implementation NOT AUTHORIZED |
