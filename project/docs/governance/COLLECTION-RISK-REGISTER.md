...
| COL-RISK-G03 | External-settlement transition/evidence ve unapplied payment lifecycle'ı runtime'da eksik (overpayment ile örtük eşitlik riski) | COL/OD-06 Option A candidate-first/orthogonal contract ile COL/OD-06A Option C hybrid typed evidence + dedicated `bank.settlement.verify` authority contract'ı RECORDED. W2.2A PR #1332 / `88290071`: exact enum + nullable/defaultsuz candidate status, no backfill, legacy `NULL` unknown/not-SETTLED. W2.2B PR #1347 / `61b49ce0`: yeni `INCOMING` receipt `PENDING`, `OUTGOING` lifecycle dışı; tenant-scoped detection-only `tryAutoMatch`; duplicate sync/backfill yok ve finansal etki sıfır. W2.2C-0 PR #1353 / `758f6186`: yeni canonical Collection create için `SETTLED` zorunlu; `PENDING`, `REJECTED` ve legacy `NULL` fail-closed; Collection/journal/event/outbox/ledger/allocation/overpayment/ClaimItem/bank-match projection write'ı sıfır; `SETTLED` canonical yol ve guard-öncesi başarılı replay korunur. Settlement evidence schema/writer, verifier permission consumer, status transition, `externalSettledAt`, canonical confirmation lifecycle ve application lifecycle hâlâ yok; provider evidence finality desteğine kadar DEFERRED. | **OPEN — TRANSITION/EVIDENCE RUNTIME ABSENT** | W2.2A/W2.2B/W2.2C-0 CLOSED / CANONICAL; W2.2C decision gate SATISFIED; W2.2C-1 READY FOR OWNER GO / IMPLEMENTATION NOT AUTHORIZED; W2.3 `BLOCKED — W2.2 BOUNDARY PENDING` |
