...
| COL-RISK-G07 | RECEIVABLE–COLLECTION legal-application persistence ve cutover zinciri tamamlanmadı | XD-001 authority sınırını; TPA-02 Option D independent aggregate'i; TPA-03 Option B ise `LegalApplicationBatch`, immutable `LegalApplication`, non-authoritative `ApplicationAttribution`, exact enum/amount/replay/reversal/bucket-identity ve two-file additive schema-foundation kontratını canonical hale getirir. ClaimItem target/payment-state/allocation authority değildir. `ClaimItem.collectedAmount` frozen legacy cache; `CollectionAllocation` canonical-output-derived transitional projection; `LedgerAllocation` historical legacy record'dur. Exact foundation scope `schema.prisma` + tek `migration.sql` olmakla birlikte schema/migration henüz uygulanmamış; writer, representative replay/evidence, consumer cutover ve retirement yoktur. | **OPEN — PHYSICAL MODEL + FOUNDATION CONTRACT DECIDED / IMPLEMENTATION + CUTOVER + RETIREMENT REMAIN** | ACT-28 / REC-AUTH-011/012 OPEN; `codex/rcv-ws04-p03-syn-01` TPA-03A schema foundation için NON-BLOCKING, writer/evidence/cutover için BLOCKING; PR #407 HOLD / CONFLICTING / DO NOT MERGE / DO NOT REBASE; representative evidence + cutover authority blocker; TPA-03A OWNER GO-IMPLEMENT REQUIRED / implementation authority NONE |
