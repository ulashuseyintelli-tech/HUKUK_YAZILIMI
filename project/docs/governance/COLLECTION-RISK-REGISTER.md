...
| COL-RISK-G07 | RECEIVABLE–COLLECTION legal-application persistence sınırı açık | XD-001 Receivable bucket/policy ownership, Collection receipt/execution orchestration ownership ve tek-yazıcı cross-domain boundary'yi canonical hale getirir. ClaimItem target/payment-state/allocation authority değildir; collectedAmount yeni reader/writer'a kapalı; CollectionAllocation bağımsız authority değildir. Fiziksel persistence owner/aggregate, writer, migration, cutover ve retirement seçilmemiştir. | **OPEN — AUTHORITY BOUNDARY DECIDED / PHYSICAL PERSISTENCE + CUTOVER REMAIN** | ACT-28 / REC-AUTH-011/012 OPEN; TPA-02 GO-ANALYZE REQUIRED; ApplicationBatch yalnız seçilmemiş analiz alternatifi; implementation authority NONE |
